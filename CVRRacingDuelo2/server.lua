...
function challengePlayer(plr,cmd,challenger,price)
    if plr and challenger then
        if getElementData(plr,"tased") then return end
                ...
                if (interior == 0 and interior2 == 0) then
                    if money >= price then
                        if money2 >= price then
                            exports['NGMessages']:sendClientMessage ("#81F7F3El jugador #5882FA"..getPlayerName(plr).." #81F7F3retó a#5882FA " ..getPlayerName(players[1]).."#81F7F3 a un touge!",root,255,0,0)
                            outputChatBox("#FF0000[CVR]#FFFFFF Has retado a "..getPlayerName(players[1]).."#FFFFFF a un touge por #00FF00$"..price,plr,255,255,255,true)
                            outputChatBox("#FF0000[CVR]#FFFFFF "..getPlayerName(plr).."#FFFFFF te ha retado a un touge por #00FF00$"..price,players[1],255,255,255,true)
                            outputChatBox("#FF0000[CVR]#FFFFFF Para aceptar el touge usa #00ff00/aceptartouge2",players[1],255,255,255,true)
...
addCommandHandler("touge2",challengePlayer)
...
addCommandHandler("aceptartouge2",acceptPlayer)
function forcereto(plr,_,plr2,plr3)
    executeCommandHandler ("touge2", getPlayerFromName(plr2), plr3 )
    executeCommandHandler("aceptartouge2",getPlayerFromName(plr3))
end
addCommandHandler("forzartouge2",forcereto)
    ...
    function()
        ...
        if opp and isElement(opp) then
            if g_raceReady[opp] == true then
                ...
                triggerEvent("onRaceStarting",resourceRoot,source,opp)
            else
...
addEventHandler('onRaceRoundStarting',root,
    function(plr1,plr2)
        ...
        if g_raceSpawn[plr1] and g_raceSpawn[plr2] then
            ...
            g_raceCars[plr1]=createVehicle(562,s1.x,s1.y,s1.z,0,0,280)
            g_raceCars[plr2]=createVehicle(562,s2.x,s2.y,s2.z,0,0,280)
...
addEventHandler('onRaceRoundStarted',root,
    function(plr1,plr2)
        triggerClientEvent ( plr1, "CreateClientCheckpoints", plr1, "CreateElements", getElementDimension ( plr1 )  )
        triggerClientEvent ( plr2, "CreateClientCheckpoints", plr2, "CreateElements", getElementDimension ( plr2 )  )
