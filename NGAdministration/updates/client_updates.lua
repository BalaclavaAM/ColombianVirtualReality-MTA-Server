...
addEventHandler ( "NGAdmin:Modules->Updates:OnServerSendClientUpdateList", root, function ( data )
	...
	if ( #data == 0 ) then
		local r = guiGridListAddRow ( updates.list )
		guiGridListSetItemText ( updates.list, r, 2, "No hay actualizaciones recientes", true, true )
	else
...
addCommandHandler ( "actualizaciones", function ( )
...
function buildManagerWindow ( )
	manager.window = guiCreateWindow(sx/2-641/2, sy/2-382/2, 641, 382, "Gestor de Actualizaciones", false)
	guiWindowSetSizable(manager.window, false )
	manager.add.lbl = guiCreateLabel(10, 26, 306, 31, "Añadir una actualización\n_____________________________________________", false, manager.window)
	manager.add.label_date = guiCreateLabel(10, 82, 91, 23, "Fecha:", false, manager.window)
	...
	manager.add.label_desc = guiCreateLabel(10, 173, 91, 23, "Descripción:", false, manager.window)
	manager.add.memo_desc = guiCreateMemo(11, 199, 305, 137, "", false, manager.window)
	manager.remove.lbl = guiCreateLabel(316, 26, 306, 31, "Remover una actualización\n___________________________________________", false, manager.window)
	manager.add.add = guiCreateButton(10, 346, 114, 29, "Añadir una actualización", false, manager.window)
	manager.remove.list = guiCreateGridList(323, 63, 309, 273, false, manager.window)
	guiGridListAddColumn(manager.remove.list, "Descripción", 0.9)
	guiGridListSetSortingEnabled ( manager.remove.list, false )
	manager.remove.remove = guiCreateButton(518, 346, 114, 26, "Remover una actualización", false, manager.window)
	manager.exit = guiCreateButton(264, 348, 114, 24, "Salir", false, manager.window)
