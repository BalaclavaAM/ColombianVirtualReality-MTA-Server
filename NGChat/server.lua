		...
		if not(cooldownadmin[player]) or getTickCount()-cooldownadmin[player]>1500 then
			...
			for k, playere in ipairs(getElementsByType("player")) do
				if exports['NGAdministration']:isPlayerStaff ( playere ) then
					local level = exports['NGAdministration']:getPlayerStaffLevel(player)
					outputChatBox("#FFFFFF[#FFFFFFAdmins #FFEC08C#0808FFV#FF0F00R#FFFFFF]#939393:".." |"..level.."| "..getPlayerName(player).."  #FFFFFF"..table.concat({...}," "),playere,0,255,0,true)	
				end
			end
		else
...
addCommandHandler("chatclub",chatclub)
