...
cancionactual["stream_title"]="None"
...
addEventHandler("onClientRender",root, function()
	...
	if ( renderText ) then
		if isElement(song) and getElementType(song)=="sound" then
			local info = getSoundMetaTags(song) 
			for i,v in pairs(info) do 
				cancionactual[i]=v 
			end 
			dxDrawBoarderedText ( "FPS Actuales: "..tostring(temp).." | Pico de FPS: "..tostring(peak).." | Ping Actual: "..tostring(ping) .. " | Ping más bajo: "..tostring(maxPing).." | Canción en RADIO CVR: "..tostring(cancionactual["stream_title"]), 15, sy/1.03, 0, 0)
		else
			dxDrawBoarderedText ( "FPS Actuales: "..tostring(temp).." | Pico de FPS: "..tostring(peak).." | Ping Actual: "..tostring(ping) .. " | Ping más bajo: "..tostring(maxPing).." |", 15, sy/1.03, 0, 0)
		end
	end
end )
