...
function saveTombo(player)
	if getElementData(player,"Job")=="Police Officer" or getElementData(player,"Job")=="Detective" then
		tombos[player]=true
	end
end
...
function shotHandler()
	outputChatBox("Amigo disparaste un arma así que desactivamos tu spawn protection ;)",source,255,0,0,false)
	...
	removeEventHandler("onPlayerWeaponFire",source,shotHandler)
end

function toRespawn ( )
	exports['NGLogs']:outputActionLog ( getPlayerName ( source ).." died" )
	if (exports['CVRDuelos']:isPlayerInDuel(source) ) then return end
	local weapons = { }
...
addEvent("NGHospitals:RespawnMk",true)
addEventHandler("NGHospitals:RespawnMk",root,toRespawn)

addEventHandler( "onResourceStart", resourceRoot,
	function()
    	for k,v in ipairs(getElementsByType("player")) do
			addEventHandler ( 'onPlayerWasted', v, toRespawn )
		end
	end
)
...
addEventHandler("onPlayerLogin", root,
  function()
    addEventHandler ( 'onPlayerWasted', source, toRespawn )
  end
)

addEventHandler("onPlayerQuit", root,
  function()
    removeEventHandler ( 'onPlayerWasted', source, toRespawn )
  end
)



function disablePlayerRespawn()
	local momos = removeEventHandler( 'onPlayerWasted',source,toRespawn)
	if momos then
		outputDebugString("Quitao "..getPlayerName(source),3,0,255,0)
	end
end
addEvent("NGHospitals:disableRespawn",true)
addEventHandler("NGHospitals:disableRespawn",root,disablePlayerRespawn)

function enableRespawn()
	outputDebugString("Activao",3,0,255,0)
	addEventHandler("onPlayerWasted",source,toRespawn)
end
addEvent("NGHospitals:enableRespawn",true)
addEventHandler("NGHospitals:enableRespawn",root,enableRespawn)
...
addEventHandler ( "NGHospitals:triggerPlayerSpawn", root, function ( d )
	...
	if tombos[source] then
		giveWeapon(source,23,1000)
		tombos[source]=nil
	end
	...
	addEventHandler( "onPlayerWeaponFire", source, shotHandler)
	...
	setTimer ( function ( source )
		...
		outputChatBox ( "La protección del spawn termina en 3...", source, 0, 255, 255 )
		setTimer ( function ( source )
			if ( not isElement ( source ) ) then return end
			outputChatBox ( "La protección del spawn termina en 2...", source, 0, 255, 255 )
			setTimer ( function ( source )
				if ( not isElement ( source ) ) then return end
				outputChatBox ( "La protección del spawn termina en 1...", source, 0, 255, 255 )
				setTimer ( function ( source )
					if ( not isElement ( source ) ) then return end
					outputChatBox ( "La protección del spawn terminó! Ahora puedes morir.", source, 255, 0, 0 )
					setElementData ( source, "isGodmodeEnabled", false )
					setElementData ( source, "isSpawnProtectionEnabled", false )
					setElementAlpha ( source, 255 )
