...
addEventHandler( 'onClientResourceStart', resourceRoot, function()
  hc_gui['main'] = guiCreateWindow( scX-310, scY/2-450/2, 300, 450, 'Creando casa', false );
  guiSetAlpha( hc_gui['main'], 1.0 );
  --Entrance--
  hc_gui['label_ec'] = guiCreateLabel( 0.042, 0.05, 0.4, 0.1, 'Coordenadas entrada', true, hc_gui['main'] );
  ...
  hc_gui['btn_ecPick'] = guiCreateButton( 0.063, 0.36, 0.39, 0.06, 'Establecer tus coordenadas', true, hc_gui['main'] ); --1
  
  --Entrance teleport--
  hc_gui['label_etc'] = guiCreateLabel( 0.009, 0.45, 0.5, 0.1, 'Coordenadas tp entrada', true, hc_gui['main'] );
  ...
  hc_gui['btn_etcPick'] = guiCreateButton( 0.063, 0.76, 0.39, 0.06, 'Establecer tus coordenadas', true, hc_gui['main'] ); --2
  
  --Exit--
  hc_gui['label_exc'] = guiCreateLabel( 0.55, 0.05, 0.4, 0.1, 'Coordenadas Salida', true, hc_gui['main'] );
  ...
  hc_gui['btn_excPick'] = guiCreateButton( 0.55, 0.36, 0.39, 0.06, 'Establecer tus coordenadas', true, hc_gui['main'] ); --3
  
  --Exit teleport--
  hc_gui['label_extc'] = guiCreateLabel( 0.51, 0.45, 0.47, 0.1, 'Coordenadas TP salida', true, hc_gui['main'] );
  ...
  hc_gui['btn_extcPick'] = guiCreateButton( 0.55, 0.76, 0.39, 0.06, 'Establecer tus coordenadas', true, hc_gui['main'] ); --4
  ...
  hc_gui['main_add'] = guiCreateWindow( scX-310-200, (scY/2-250/2)-450/4.5, 200, 250, 'Informacion', false );
  ...
  hc_gui['label_dim'] = guiCreateLabel( 0.57, 0.249, 0.3, 0.1, 'Dimensión', true, hc_gui['main_add'] );
  guiLabelSetVerticalAlign( hc_gui['label_dim'], 'center' );
  
  hc_gui['edit_cost'] = guiCreateEdit( 0.09, 0.4, 0.45, 0.1, '', true, hc_gui['main_add'] );
  hc_gui['label_cost'] = guiCreateLabel( 0.57, 0.399, 0.3, 0.1, 'Valor $$$', true, hc_gui['main_add'] );
  ...
  addEventHandler( 'onClientGUIClick', hc_gui['btn_create'], function( btn )
    ...
    for i, v in pairs( hc_edit ) do
      if guiGetText( v ) == '' then
        outputChatBox( '* Debes rellenar todos los campos', 255, 51, 36 );
        return false;
      end;
    end;
    for i, v in pairs( hc_gui ) do
  ...
  hm_gui['main'] = guiCreateWindow( scX-360, scY-360, 350, 350, 'Panel del hogar', false );
  guiSetAlpha( hm_gui['main'], 1.0 );
  
  hm_gui['label_buy'] = guiCreateLabel( 0.03, 0.08, 0.255, 0.1, 'Comprar hogar', true, hm_gui['main'] );
  ...
  hm_gui['label_sale'] = guiCreateLabel( 0.365, 0.08, 0.255, 0.1, 'Vender hogar', true, hm_gui['main'] );
  ...
  hm_gui['label_enter'] = guiCreateLabel( 0.03, 0.45, 0.265, 0.1, 'Entrar a la casa', true, hm_gui['main'] );
  ...
  hm_gui['label_newowner'] = guiCreateLabel( 0.365, 0.45, 0.255, 0.1, 'Establecer dueño', true, hm_gui['main'] );
  ...
  hm_gui['label_destroy'] = guiCreateLabel( 0.68, 0.45, 0.295, 0.1, 'Destruir casa', true, hm_gui['main'] );
  ...
  hm_gui['label_ID'] = guiCreateLabel( 0.05, 0.82, 0.25, 0.1, 'Casa Nº: ', true, hm_gui['main'] );
  hm_gui['label_owner'] = guiCreateLabel( 0.05, 0.88, 0.25, 0.1, 'Dueño: ', true, hm_gui['main'] );
  hm_gui['label_price'] = guiCreateLabel( 0.4, 0.82, 0.25, 0.1, 'Valor $: ', true, hm_gui['main'] );
  ...
  addEventHandler( 'onClientGUIClick', root, function( btn )
    ...
    if source == hm_gui['icon_buy'] then
      ...
      guiSetText( hm_gui['onBuy_edit'], 'Clave' );
      playSound( 'sounds/click_icon.wav' );
      guiSetInputEnabled( true );
    elseif source == hm_gui['icon_sale'] then
      guiSetVisible( hm_gui['main'], false );
      guiSetVisible( hm_gui['wnd_key_onSale'], true );
      guiSetText( hm_gui['onSale_edit'], 'Clave' );
      playSound( 'sounds/click_icon.wav' );
      guiSetInputEnabled( true );
    elseif source == hm_gui['icon_key'] then
      guiSetVisible( hm_gui['main'], false );
      guiSetVisible( hm_gui['wnd_key_onChange'], true );
      guiSetText( hm_gui['onChange_edit_Old'], 'Clave vieja' );
      guiSetText( hm_gui['onChange_edit_New'], 'Nueva clave' );
      guiSetText( hm_gui['onChange_edit_RepeatNew'], 'Nueva clave' );
      playSound( 'sounds/click_icon.wav' );
      guiSetInputEnabled( true );
    elseif source == hm_gui['icon_newowner'] then
      ...
      guiSetText( hm_gui['onNewOwner_nameEdit'], 'Nombre cuenta' );
      playSound( 'sounds/click_icon.wav' );
      guiSetInputEnabled( true );
  ...
  --Buy
  hm_gui['wnd_key_onBuy'] = guiCreateWindow( scX/2-250/2, scY/2-150/2, 250, 150, 'Clave', false );
  guiSetAlpha( hm_gui['wnd_key_onBuy'], 1.0 );
  hm_gui['onBuy_label'] = guiCreateLabel( 0.05, 0.15, 0.9, 0.4, "Vas a comprar la casa. Escriba su clave para que nadie más pueda entrar en esta casa. ¡No lo olvides!", true, hm_gui['wnd_key_onBuy'] ); 
  ...
  hm_gui['onBuy_confirm'] = guiCreateButton( 0.2, 0.75, 0.2, 0.15, 'Confirmar', true, hm_gui['wnd_key_onBuy'] );
  hm_gui['onBuy_cancel'] = guiCreateButton( 0.57, 0.75, 0.2, 0.15, 'Cancelar', true, hm_gui['wnd_key_onBuy'] );
  guiSetVisible( hm_gui['wnd_key_onBuy'], false );
  
  --Sale
  hm_gui['wnd_key_onSale'] = guiCreateWindow( scX/2-250/2, scY/2-150/2, 250, 150, 'Clave', false );
  guiSetAlpha( hm_gui['wnd_key_onSale'], 1.0 );
  hm_gui['onSale_label'] = guiCreateLabel( 0.05, 0.15, 0.9, 0.4, "Usted va a vender esta casa a la mitad de su precio. Ingresa tu clave para confirmar que eres el verdadero propietario.", true, hm_gui['wnd_key_onSale'] ); 
  ...
  hm_gui['onSale_confirm'] = guiCreateButton( 0.2, 0.75, 0.2, 0.15, 'Confirmar', true, hm_gui['wnd_key_onSale'] );
  hm_gui['onSale_cancel'] = guiCreateButton( 0.57, 0.75, 0.2, 0.15, 'Cancelar', true, hm_gui['wnd_key_onSale'] );
    ...
    if source == hm_gui['onBuy_confirm'] then
      ...
      if #passTxt >= getElementData( me, 'k_len' ) then
        if not passTxt:find( ' ' ) then
          ...
          outputChatBox( '* La clave no puede tener espacios', 255, 51, 36 );
        end;
      else
  ...
  addEventHandler( 'onClientGUIClick', root, function( btn )
    ...
    if source == hm_gui['onSale_confirm'] then
      ...
      if guiGetText( hm_gui['onSale_edit'] ) == getElementData( mrk, 'HS_INFO' )[8] then
        ...
        outputChatBox( '* Has vendido tu casa', 255, 255, 0 );
        guiSetText( hm_gui['onSale_edit'], '' );
        guiSetVisible( hm_gui['wnd_key_onSale'], false );
        playSound( 'sounds/click_confirm.wav' );
        guiSetInputEnabled( false );
      else
        outputChatBox( '* Clave erronea', 255, 51, 36 );
      end;
  ...
  hm_gui['wnd_key_onChange'] = guiCreateWindow( scX/2-250/2, scY/2-250/2, 250, 250, 'Clave', false );
  guiSetAlpha( hm_gui['wnd_key_onChange'], 1.0 );
  hm_gui['onChange_label'] = guiCreateLabel( 0.05, 0.15, 0.9, 0.4, "Para cambiar su clave, escriba su clave VIEJA y luego una NUEVA.", true, hm_gui['wnd_key_onChange'] ); 
  guiLabelSetHorizontalAlign( hm_gui['onChange_label'], 'center', true );
  guiLabelSetColor( hm_gui['onChange_label'], 0, 153, 255 );
  hm_gui['onChange_edit_Old'] = guiCreateEdit( 0.14, 0.3, 0.7, 0.1, 'Clave vieja', true, hm_gui['wnd_key_onChange'] );
  hm_gui['onChange_edit_New'] = guiCreateEdit( 0.14, 0.45, 0.7, 0.1, 'Clave nueva', true, hm_gui['wnd_key_onChange'] );
  hm_gui['onChange_edit_RepeatNew'] = guiCreateEdit( 0.14, 0.6, 0.7, 0.1, 'Clave nueva', true, hm_gui['wnd_key_onChange'] );
      ...
      if guiGetText( hm_gui['onChange_edit_Old'] ) == currKey then
        if #guiGetText( hm_gui['onChange_edit_New'] ) >= getElementData( me, 'k_len' ) then
          if guiGetText( hm_gui['onChange_edit_New'] ) == guiGetText( hm_gui['onChange_edit_RepeatNew'] ) then
            ...
            outputChatBox( '* La nueva clave es #00FF00'..guiGetText( hm_gui['onChange_edit_RepeatNew'] ), 255, 51, 36, true );
            ...
            outputChatBox( '* Las claves son distintas, tienen que ser iguales', 255, 51, 36 );
          end;
        else
          outputChatBox( '* La clave debe tener al menos '..tostring( getElementData( me, 'k_len' ) )..' caracteres', 255, 51, 36 );
        end;
      else
        outputChatBox( '* La clave antigua es incorrecta', 255, 51, 36 );
      end;
  ...
  hm_gui['wnd_key_onNewOwner'] = guiCreateWindow( scX/2-250/2, scY/2-250/2, 250, 250, "Nombre del nuevo dueño", false );
  guiSetAlpha( hm_gui['wnd_key_onNewOwner'], 1.0 );
  hm_gui['onNewOwner_label'] = guiCreateLabel( 0.05, 0.11, 0.9, 0.4, "Escriba el nombre de la cuenta del nuevo propietario y su clave ACTUAL. NOTA: después de esta operación ya no serás el dueño de esta casa", true, hm_gui['wnd_key_onNewOwner'] ); 
  ...
  hm_gui['onNewOwner_nameEdit'] = guiCreateEdit( 0.14, 0.6, 0.7, 0.1, 'Nombre', true, hm_gui['wnd_key_onNewOwner'] );
        ...
        if guiGetText( hm_gui['onNewOwner_nameEdit'] ) ~= getPlayerName( me ) then
          if getPlayerByAccountName( guiGetText( hm_gui['onNewOwner_nameEdit'] ) ) then
            if guiGetText( hm_gui['onNewOwner_keyEdit'] ) == getElementData( getHouseByID( getElementData( me, 'mrk_in' ) ), 'HS_INFO' )[8] then
              ...
              outputChatBox( '* El dueño de esta casa es #00FF00'..guiGetText( hm_gui['onNewOwner_nameEdit'] ):gsub( "%x%x%x%x%x%x", "" )..' for now!', 255, 51, 36, true );
              guiSetText( hm_gui['onNewOwner_nameEdit'], '' );
              guiSetVisible( hm_gui['wnd_key_onNewOwner'], false );
              playSound( 'sounds/click_confirm.wav' );
              guiSetInputEnabled( false );
            else
              outputChatBox( '* Clave erronea prueba otra vez', 255, 51, 36 );
              guiSetText( hm_gui['onNewOwner_nameEdit'], '' );
              guiSetText( hm_gui['onNewOwner_keyEdit'], '' );
            end;
          else
            outputChatBox( '* No se encontro un jugador', 255, 51, 36 );
          end;
        end;
      else
        outputChatBox( "* Tu nombre no puede tener espacios", 255, 51, 36 );
      end;
  ...
  hm_gui['wnd_onDestroy'] = guiCreateWindow( scX/2-250/2, scY/2-150/2, 250, 150, 'Destruir la casa', false );
  guiSetAlpha( hm_gui['wnd_onDestroy'], 1.0 );
  hm_gui['onDestroy_label'] = guiCreateLabel( 0.05, 0.23, 0.9, 0.4, 'Estas seguro de esto?', true, hm_gui['wnd_onDestroy'] ); 
  guiLabelSetHorizontalAlign( hm_gui['onDestroy_label'], 'center', true );
  guiLabelSetColor( hm_gui['onDestroy_label'], 0, 153, 255 );
  hm_gui['onDestroy_confirm'] = guiCreateButton( 0.2, 0.65, 0.2, 0.15, 'Confirmar', true, hm_gui['wnd_onDestroy'] );
  hm_gui['onDestroy_cancel'] = guiCreateButton( 0.57, 0.65, 0.2, 0.15, 'Cancelar', true, hm_gui['wnd_onDestroy'] );
  ...
  hm_gui['wnd_key_onEnter'] = guiCreateWindow( scX/2-250/2, scY/2-150/2, 250, 150, 'Clave', false );
  guiSetAlpha( hm_gui['wnd_key_onEnter'], 1.0 );
  hm_gui['onEnter_label'] = guiCreateLabel( 0.05, 0.15, 0.9, 0.4, "Vas a entrar a la casa. Escriba su clave, por favor:", true, hm_gui['wnd_key_onEnter'] ); 
  ...
  hm_gui['onEnter_confirm'] = guiCreateButton( 0.2, 0.75, 0.2, 0.15, 'Confirmar', true, hm_gui['wnd_key_onEnter'] );
  hm_gui['onEnter_cancel'] = guiCreateButton( 0.57, 0.75, 0.2, 0.15, 'Cancelar', true, hm_gui['wnd_key_onEnter'] );
  ...
  addEventHandler( 'onClientGUIClick', root, function( btn )
    ...
    if source == hm_gui['icon_enter'] then
      ...
      guiSetText( hm_gui['onEnter_edit'], 'Clave' );
      playSound( 'sounds/click_icon.wav' );
      guiSetInputEnabled( true );
