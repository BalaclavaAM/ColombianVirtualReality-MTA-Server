...
addCommandHandler( 'hpanel', function( player )
  if isObjectInACLGroup( 'user.'..getAccountName( getPlayerAccount( player ) ), aclGetGroup( 'Admin' ) ) or hasObjectPermissionTo( player, 'function.banPlayer', false ) then
    ...
    outputChatBox( '* No puedes usar este comando', player, 255, 36, 51 );
  end;
end );
...
addEventHandler( 'onPlayerAttemptCreateHouse', root, function( rt )
  ...
  outputChatBox( '* Has creado una nueva casa', client, 255, 255, 0 );
end );
    ...
    if getElementType( player ) == 'player' and not getPedOccupiedVehicle( player ) then
      if not getElementData( player, 'HP_Opened' ) then
        if not isGuestAccount( getPlayerAccount( player ) ) then
          ...
          outputChatBox( '* Debes loguearte para entrar en esta casa', player, 255, 51, 36 );
        end;
      end;
    end;
  end );
...
function onPlayerHouseMarkerHit( player, mrk, cursor )
  ...
  if isGuestAccount( acc ) then
    outputChatBox( '* Debes iniciar sesión para entrar a esta casa', player, 255, 51, 36 );
    setElementData( player, 'mrk_in', nil );
...
addEventHandler( 'HOUSE_Buy', root, function( cost, key )
  ...
  if houseCounter >= tonumber( get( 'playerHouseCounter' ) ) then
    outputChatBox( '* No puedes comprar mas de #00FF00'..get( 'playerHouseCounter' )..' casa(-s)#FF3324', client, 255, 51, 36, true );
    setPedFrozen( client, false );
    return false;
  end;
  if getPlayerMoney( client ) >= tonumber( cost ) then
    outputChatBox( '* Contraseña establecida #00FF00'..key, client, 255, 51, 36, true );
    outputChatBox( '* Has comprado la casa', client, 255, 255, 0 );
    ...
    outputChatBox( '* No tienes el suficiente dinero', client, 255, 51, 36 );
...
addEventHandler( 'HOUSE_ChangeOwner', root, function( name )
  ...
  setElementData( mrk, 'HS_INFO', { t[1], t[2], t[3], t[4], t[5], t[6], t[7], t[8], t[9] } );
  setTimer( onPlayerHouseMarkerHit, 50, 1, client, mrk, false );
  outputChatBox( '* #FFFF00'..getPlayerName( client )..'#00FF00 te ha dado su casa!', getPlayerFromName( name ), 0, 255, 0, true );
end );
...
addEventHandler( 'HOUSE_Destroy', root, function()
  ...
  outputChatBox( '* La casa #'..getElementData( source, 'mrk_in' )..' ha sido eliminada', source, 255, 255, 0 );
