  ...
  if isObjectInACLGroup( 'user.'..getAccountName( getPlayerAccount( player ) ), aclGetGroup( 'Admin' ) ) or hasObjectPermissionTo( player, 'function.banPlayer', false ) then
    if getElementData( player, 'HP_Opened' ) then
      ...
      if r and r > 0 and r <= #ints then
        ...
        outputChatBox( '* ID de interior incorrecta', player, 255, 51, 36 );
      end;
    else
      outputChatBox( '* Debes tener el panel de construccion abierto para este comando.', player, 255, 51, 36 );
    end;
  else
    outputChatBox( '* No puedes usar este comando', player, 255, 51, 36 );
  end;
end );