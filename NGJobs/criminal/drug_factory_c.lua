...
function DR.onMarkerHit ( p )
	...
	if ( tostring ( getElementData ( p, "Job" ) ):lower() ~= "criminal" ) then
		return exports.ngmessages:sendClientMessage ( "Solo criminales pueden usar esta función", 255, 80, 80 );
	end
	
	
	DR.GUI.window = guiCreateWindow((sx/2-572/2), (sy/2-269/2), 572, 269, "Selección", false)
	guiWindowSetSizable(DR.GUI.window, false)

	DR.GUI.info = guiCreateLabel(19, 28, 391, 23, "Qué tipo de droga te gustaría producir?", false, DR.GUI.window)
	...
	DR.GUI.progMsg = guiCreateLabel ( 0, 0, 500, 60, "Obteniendo ", false, DR.GUI.prog );
...
function DR.onMarkerLeave ( p )
	...
	if ( DR.gathering.current ) then 
		...
		exports.ngmessages:sendClientMessage ( "El proceso de producción ha fallado "..DR.gathering.item.."!", 255, 80, 80 );
...
function DR.onClientGUIClick ( )
	...
	elseif ( source == DR.GUI.btn_weed or source == DR.GUI.btn_LSD ) then
		...
		if ( source == DR.GUI.btn_weed ) then 
			...
			guiSetText ( DR.GUI.progMsg, "Obteniendo Marihuana.... 0%" );
		elseif ( source == DR.GUI.btn_LSD ) then
			DR.gathering.item = "LSD";
			guiSetText ( DR.GUI.progMsg, "Obteniendo LSD.... 0%" );
		end
...
function DR.onDrugPercentIncrease ( )
	...
	guiSetText ( DR.GUI.progMsg, "Obteniendo "..DR.gathering.item..".... "..DR.gathering.percent.."%" );
	...
	if ( DR.gathering.percent >= 100 ) then 
		...
		triggerServerEvent ( "NGJobs->Criminal->DrugFactory->OnPlayerFinishProducingDrugs", localPlayer, DR.gathering.item, 4 );
		exports.ngmessages:sendClientMessage ( "Has terminado de obtener 4 "..DR.gathering.item.." drogas!", 80, 255, 80, img, false );
