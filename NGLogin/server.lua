...
function attemptRegister ( user, pass, email )
	if ( user and pass and email and type ( user ) == 'string' and type ( pass ) == 'string' and type (email) == 'string' ) then
		local q = exports.NGSQL:email_exist ( email )
		--local user = string.lower ( user )
		--local pass = string.lower ( pass )
		local account = getAccount ( user )
		if ( not account and not q ) then
			...
			if ( account ) then
				...
				exports['NGSQL']:createAccount ( user, email );
...
addEventHandler ( "Login:onPlayerFinishIntro", root, function ( )
	if source then
		...
		setElementData(source, "NGUser.UnemployedSkin", getElementData(source,"skin"))
