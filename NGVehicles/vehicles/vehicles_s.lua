addEventHandler ( "onResourceStart", resourceRoot, function ( )
	exports['NGSQL']:db_exec ( "CREATE TABLE IF NOT EXISTS vehicles ( Owner TEXT, VehicleID INT, ID INT, Color TEXT, Upgrades TEXT, Position TEXT, Rotation TEXT, Health TEXT, Visible INT, Fuel INT, Impounded INT, Handling TEXT, NEON TEXT, PAINTJOB TEXT, FLAGSELL INT, Partes TEXT )" )
...
function showVehicle ( id, stat, player, msg )
	if stat then
		...
		if ( not isElement ( vehicles[id] ) ) then
			...
			if ( q and type ( q ) == 'table' and #q > 0 ) then
				...
				for i, v in ipairs ( upgrades ) do 
					...
					if tonumber(v) == 1008 then
						setElementData(vehicles[id], "tuning.nitroLevel", 50)
					elseif tonumber(v) == 1009 then
						setElementData(vehicles[id], "tuning.nitroLevel", 75)
					elseif tonumber(v) == 1010 then
						setElementData(vehicles[id], "tuning.nitroLevel", 100)
					end
				end
		...
		if ( isElement ( vehicles[id] ) ) then
			...
			exports['NGSQL']:db_exec ( "UPDATE vehicles SET Color=?, Upgrades=?, Position=?, Rotation=?, Health=?, Fuel=?, Handling=?, NEON=?, PAINTJOB=?, FLAGSELL=?, Partes=? WHERE VehicleID=?", color, upgrades, pos, rot, health, fuel, hdnl, neon, paintjob, flagsell, toJSON(partes), id )
			setElementData(vehicles[id], "vehicle.neon.active", false)
...
function givePlayerVehicle ( player, vehID, r, g, b ) 
	...
	exports['NGSQL']:db_exec ( "INSERT INTO `vehicles` (`Owner`, `VehicleID`, `ID`, `Color`, `Upgrades`, `Position`, `Rotation`, `Health`, `Visible`, `Fuel`, `Impounded`, `Handling`,`NEON`,`PAINTJOB`,`FLAGSELL`,`Partes`) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?);", getAccountName(getPlayerAccount(player)), tostring(id), tostring(vehID), color, upgrades, pos, rot, health, '100', '0', '0', toJSON ( getModelHandling ( vehID ) ),"None",3,0,toJSON({}) )
	return id
end
...
function getAccountVehicles ( account )
	...
	if ( type ( query ) == 'table' and #query >= 1 ) then
		...
		for i, v in pairs ( query ) do 
			table.insert ( rV, { v['Owner'], v['VehicleID'], v['ID'], v['Color'], v['Upgrades'], v['Position'], v['Rotation'], v['Health'], v['Visible'], v['Fuel'], v['Impounded'], v['Handling'], v['NEON'],v['PAINTJOB'],v['FLAGSELL'] } )
		end
		return rV
	else
		return { }
...
function recoverVehicle ( source, id )
	...
	if ( getPlayerMoney ( source ) >= rPrice ) then
		...
		exports['NGMessages']:sendClientMessage ( "Your vehicle has been recovered to the "..getZoneName (2374.1596679688, -2267.5627441406, 5.6673183441162, true ).." recovery shop for $"..tostring(rPrice).."!", source, 0, 255, 0 )
...
function giveVehicle( ID, player )
	if isElement(player) then
		...
		if cuenta then
			...
			exports['NGLogs']:outputVehicleLog ( getPlayerName (source).." envió el vehículo con ID: "..tostring(ID).." a "..getPlayerName(player).." correctamente." )
			outputChatBox("El jugador "..getPlayerName(source).." te transfirió el vehículo con ID "..tostring(ID).." satisfactoriamente.",player,255,255,0)
	...
	if (vehicleparts[car]) then
		if not(repairsPetitions[duenho]) then
			...
			elseif partetofix=="bump_front_dummy" then
				parte="Rueda izquierda delantera"
			elseif partetofix=="bump_rear_dummy" then
