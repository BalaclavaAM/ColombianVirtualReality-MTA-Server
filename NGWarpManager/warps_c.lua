...
function onWarpHit ( p, matchingDimension )
	if ( source.dimension == p.dimension and source.interior == p.interior and p == localPlayer and matchingDimension ) then
		hitWarp = source
...
addEventHandler ( "onClientPreRender", root, function ( )
	...
	if ( hitWarp ) then
		...
		if ( hitWarp and not warping ) then
			dxDrawBoarderedText ( "Presiona Shift Para Warpearte", 0, sy*600, sx_, sx*620, tocolor ( 0, 255, 0, 255 ), sy*1.5, "pricedown", "center", "top", nil, nil, nil, nil, nil, nil, nil, nil, 1.5 )
		end
	end 
end )
