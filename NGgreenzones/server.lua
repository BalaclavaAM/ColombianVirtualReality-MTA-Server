...
local greenzones = {
	...
	{x=1149.8863525391, y=-1392.8568115234 , width=220.09655761719, depth=117.50390625}, -- Hospital Central
	...
	{x=354.17, y=158.64, width=200, dimension=1, interior=3, depth=200, height=2000},--Banco LS
						...
						addEventHandler("onColShapeLeave", c,
							function(h)
								if h and isElement(h) and getElementType(h) == "player" then
									...
									if getElementData(h,"Job")=="Police Officer" or getElementData(h,"Job")=="Criminal" or getElementData(h,"Job")=="UnEmployed" then
							...
							function(h)
								if h and isElement(h) and getElementType(h) == "vehicle" then
									...
									if table.size(ocupantes)>0 then
										for silla,ocupante in pairs(ocupantes) do
											if getElementData(ocupante,"Job")=="Police Officer" or getElementData(ocupante,"Job")=="Criminal" or getElementData(ocupante,"Job")=="UnEmployed" then
