	...
	if getPlayerCount() >= 3 then
		...
		while (true) do 
			if i ~= 20 then
				...
				if getElementInterior(cPlayer) == 0 and not isGuestAccount(acc) then
					break
				end
				i = i + 1
			else
		...
		preventTimer = setTimer(
			function()
				if getElementInterior(cPlayer) ~= 0 or getElementDimension(cPlayer) ~= 0 then
					...
					exports['NGMessages']:sendClientMessage ("Has perdido por cambiar de interior o dimension.",cPlayer,255,0,0)
					cPlayer = false
					if isElement(cBlip) then
						destroyElement(cBlip)
						cBlip = nil
...
function trabajadores(attacker,damage_causing)
	if attacker and (attacker ~= source) and (getElementType(attacker)=="player" ) then
		...
		if (equipoatacado == "Services") and (getPlayerWantedLevel (source) == 0) and (equipoatacante~="Emergency") then
