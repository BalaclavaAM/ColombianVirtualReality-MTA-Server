...
function notifyPlayerLoggedIn(player)
	outputChatBox ( "Presiona p para abrir tu panel admin", player )
	local unread = 0
...
function checkNickOnChange(old, new)
	if aNickChangeTime[source] and aNickChangeTime[source] + tonumber(get("*nickChangeDelay")) > getTickCount() then
		cancelEvent()
		outputChatBox("Sólo te puedes cambiar el nombre cada "..(tonumber(get("*nickChangeDelay"))/1000).." segundos", source, 255, 0, 0)
		return false
	else
