...
function crearPanel2()
    ...
    CVRCarrerasNomCarR = guiCreateLabel(297, 47, 158, 15, "Penetrar", false, CVRCarrerasPanelMain)
...
function creadorPanel()
    if isElement(CVRCarrerasPanelMain) then
        ...
        addEventHandler("onClientGUIClick",CVRCarrerasPanelMain,clickPanelCarreras)
        showCursor(true)
    end
end
addCommandHandler("cargarcarrera",creadorPanel)

function abrirconMarker(he,md)
    if he==localPlayer and md then
        creadorPanel()
    end
end

local marcador=createMarker(1319.42, -1319.51, 13.63-1, "cylinder", 2, 255,255,255, visibleTo)
addEventHandler("onClientMarkerHit",marcador,abrirconMarker)