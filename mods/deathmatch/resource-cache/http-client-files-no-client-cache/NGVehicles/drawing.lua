...
local triggeredCar=false
local triggeredNoCar=false

tripData = {vehicle=false,x=0,y=0,traveledDistance=0 }

function handleSaveVehicle(vehicleSaved,carID)
	if vehicleSaved==tripData['vehicle'] then
		triggerServerEvent("NGVehicles:SaveKmServer",localPlayer,carID,tripData['traveledDistance'])
	end
end
addEvent("NGVehicles:RequestKilometraje",true)
addEventHandler("NGVehicles:RequestKilometraje",root,handleSaveVehicle)
...
function drawing()
	if tostring(state) == "true" then
		...
		if vh then
			...
			if not(triggeredCar) then
				triggeredCar=true
				triggeredNoCar=false
				triggerEvent("imageCar",localPlayer,true)
			end
			...
			if not(vh==tripData['vehicle']) then
				tripData['vehicle']=vh
				tripData['x']= x
				tripData['y']= y
				tripData['traveledDistance']=getElementData(vh,"kilometraje") or 0 
			end
			...
			if speed>1 then
				tripData['traveledDistance'] = (getDistanceBetweenPoints2D(tripData['x'],tripData['y'],x,y)/1000) + tripData['traveledDistance']
				tripData['x']=x
				tripData['y']=y
			end
			...
			dxDrawText(math.floor(tripData['traveledDistance']).." Km", ivX, ivY, ivSX, ivSY, tocolor(255, 255, 255, 255), infofont+0.2, "default-bold", "right", "center", false, false, false, true, true) 		
			...
			if not(triggeredNoCar) then
				triggeredCar=false
				triggeredNoCar=true
				triggerEvent("imageCar",localPlayer,false)
			end
		end
	end	
end
