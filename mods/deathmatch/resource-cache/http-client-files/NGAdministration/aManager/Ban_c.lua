...
local admin = "Balaclava"
local reason = "e"
local currentwarns = 1
local totalwarns = 1

local screenW, screenH = guiGetScreenSize()
function dibujarWarneo()
	dxDrawRectangle(screenW * 0.3115, screenH * 0.1942, screenW * 0.3823, screenH * 0.2275, tocolor(252, 40, 219, 105), false)
	dxDrawText("CVR Warnings", screenW * 0.4109, screenH * 0.2025, screenW * 0.5932, screenH * 0.2425, tocolor(255, 255, 255, 255), 1.00, "pricedown", "center", "top", false, false, false, false, false)
	dxDrawText("Has sido warneado por el administrador \n"..admin.." debido a \n"..reason.."\n("..currentwarns.."/3). Total warneos ("..totalwarns..").", screenW * 0.3797, screenH * 0.2508, screenW * 0.6380, screenH * 0.3267, tocolor(255, 255, 255, 255), 0.75, "bankgothic", "center", "top", false, false, false, false, false)   
end

function onWarnPlayer(aadmin,rreason,ccurrentwarns,ttotalwarns)
	admin=aadmin
	reason=rreason
	currentwarns=ccurrentwarns
	totalwarns=ttotalwarns
	addEventHandler("onClientRender",root,dibujarWarneo)
	if isElement(xd) then
		destroyElement(xd)
	end
	xd = guiCreateButton(0.47, 0.38, 0.07, 0.03, "Okay", true)
	guiSetProperty(xd, "NormalTextColour", "69FC28DB") 
	showCursor(true)
	addEventHandler("onClientGUIClick",xd,function() removeEventHandler("onClientRender",root,dibujarWarneo) destroyElement(xd) showCursor(false) end,false)
end
addEvent("onPlayerWarnClient",true)
addEventHandler("onPlayerWarnClient",root,onWarnPlayer)
