...
function pilot.createFlightPath ( )
	...
	addEventHandler ( "onClientMarkerHit", flightPath.elements.FlightMarker, function ( p )
		if ( p == localPlayer and isPedInVehicle ( p ) ) then
			...
			if ( pilot.VehicleSpeed ( c ) > 10 ) then return exports.NGMessages:sendClientMessage("Entra al marcador a menos de 10 kilómetros por hora.",255,0,0) end
			pilot.destroyFlightPath ( )
			local jobRanked = getElementData(localPlayer, "Job Rank")
			mult = 0.5
			if jobRanked == "Aspirante" then
				mult = 0.5
			elseif jobRanked == "Asistente" then
				mult = 0.6
			elseif jobRanked == "Segundo Oficial" then
			mult = 0.65
			elseif jobRanked == "Primer Oficial" then
				mult = 0.7
			elseif jobRanked == "Capitan" then
				mult = 0.75
			elseif jobRanked == "Primer Oficial Comercial" then
				mult = 0.80
			elseif jobRanked == "Capitan Comercial" then
				mult = 0.85
			elseif jobRanked == "Comandante Comercial" then
				mult = 0.9
			elseif jobRanked == "Maestro De Vuelo" then
				mult = 1
			elseif jobRanked == "Veterano" then
				mult = 1.05
			end
			local m = math.floor(math.random ( 25000, 35000 )*mult)
	...
	if ( pilot.ShouldiGetaGoodPoint ( ) ) then
		...
		addEventHandler ( "onClientMarkerHit", flightPath.elements.FlightMarkerExtra, function ( p )
			if ( p == localPlayer and isPedInVehicle ( p ) ) then
				...
				if ( getElementData ( c, "NGAntiRestart:VehicleJobRestriction" ) == nil ) then return end
				if ( getVehicleController ( c ) ~= p ) then return end
				if ( pilot.VehicleSpeed ( c ) > 15 ) then return exports.NGMessages:sendClientMessage("Enter the marker at a slower pace.",255,0,0) end
				pilot.destroyFlightPath ( )
				local m = 4500
				triggerServerEvent ( "NGJobs->GivePlayerMoney", localPlayer, localPlayer, m, "You were paid $"..tostring(m).." for completing the flight path!" )
				triggerServerEvent ( "NGJobs->SQL->UpdateColumn", localPlayer, localPlayer, "completeflights", "AddOne" )
