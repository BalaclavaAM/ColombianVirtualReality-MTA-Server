...
function createSellWindow ( ) 
	...
	guiGridListAddColumn(sell.list, "Visible", 0.15)
	guiGridListAddColumn(sell.list,"Modificado",0,15)
	guiGridListAddColumn(sell.list, "Impounded", 0.15)
	...
	for index, var in pairs ( myvehicles ) do 
		...
		local FLAGSELL = ( tostring ( var.FLAGSELL ) == "1" and "Yes" ) or "No";

		if ( visible == "Yes" or impounded == "Yes" or FLAGSELL == "Yes" ) then 
			color = { 255, 0, 0 }
		end 
		...
		guiGridListSetItemText ( sell.list, r, 3, FLAGSELL, false, false );
		guiGridListSetItemText ( sell.list, r, 4, impounded	, false, false );
		guiGridListSetItemData ( sell.list, r, 1, var );
		
		for i=1,4 do 
			guiGridListSetItemColor ( sell.list, r, i, color[1], color[2], color[3], color[4] )
		end 
