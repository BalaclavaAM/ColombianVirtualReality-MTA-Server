...
function createWindow ( ) 
	...
	gui.vDetails = guiCreateLabel(sx*14, sy*296, sx*212, sy*130, "Vehiculo: None\n\nDueño: None\n\nPrecio: None", false, gui.usedWind)
	...
	gui.recover = guiCreateButton(sx*14, sy*389, sx*170, sy*21, "Recuperar", false, gui.usedWind)

	-- Vehicle details menu
	gui.details.window = guiCreateWindow((sx_/2-(sx*478)/2), (sy_/2-(sy*386)/2), sx*478, sy*386, "Detalles", false)
...
function onGuiClick ( )
	...
	elseif ( source == gui.addVehicle ) then 
		createSellWindow ( );
	elseif ( source == gui.recover) then 
		local r, _ = guiGridListGetSelectedItem ( gui.usedList );
			if ( r == -1 ) then 
				return exports.ngmessages:sendClientMessage ( "You must select a vehicle for this action!", 200, 100, 130 );
			end 
			...
			local id = guiGridListGetItemData ( gui.usedList, r, 1 ).unique_id;
			triggerServerEvent ( "NGShops->UsedVehicles->onPlayerTryRecoverVehicle", localPlayer, id );
	end 
	...
	if ( sell ) then 
		...
		elseif ( source == sell.sell ) then 
			...
			elseif ( tostring ( data.FLAGSELL ) == "1" ) then 
				return exports.ngmessages:sendClientMessage ( "Tu vehiculo no puede estar modificado.", 200, 100, 130 );
			elseif ( table.len(fromJSON(data.Partes))>0 ) then
				return exports.ngmessages:sendClientMessage ( "Tu vehiculo le faltan partes.", 200, 100, 130 );
			elseif ( fromJSON( data.tunningInfo )['unsoldable'] ) then
				return exports['NGMessages']:sendClientMessage ( "Ese vehiculo fue comprado por dinero real. No lo puedes regalar/vender.", 255, 0, 0 )
			...
			elseif ( price < 1000 or price > 99000000 ) then 
