	...
	for _, player in ipairs(getElementsWithinRange(x, y, z, 300, "player")) do
		if ((player == localPlayer and showMyIcon) or player ~= localPlayer) and player:getData("ChatIcon:Chatting") then
			if not isElement(chatIconFor[player]) then
				chatIconFor[player] = guiCreateStaticImage(0, 0, 0, 0, "chat2.png", false) -- change to "chat2.png" to use the included, modern icon
			end
...
addCommandHandler("ocultaricono", toggleIcon, false)
...
addCommandHandler("escalaicono", resizeIcon, false)
...
addCommandHandler("alfaicono", setIconAlpha, false)
