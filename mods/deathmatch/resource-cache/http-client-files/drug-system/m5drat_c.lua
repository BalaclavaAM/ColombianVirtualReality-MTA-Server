...
function dxDrawColorText(str, ax, ay, bx, by, color, scale, font, alignX, alignY)
    ...
    if alignX then
        if alignX == "center" then
            ax =
                ax +
                (bx - ax - callClientFunction(source, "dxGetTextWidth", str:gsub("#%x%x%x%x%x%x", ""), scale, font)) / 2
    ...
    if last <= #str then
        ...
        callClientFunction(
            ...
            ax,
            ay,
            ax + callClientFunction(source, "dxGetTextWidth", cap, scale, font),
            by,
            color,
            scale,
            font
...
---------- Shop Window
buywindow = guiCreateWindow(127, 145, 563, 309, "Drug-system Shop V1.0", false)
...
memo = guiCreateMemo(185, 191, 193, 102, "Drug-System V1.0 by ahmedfef\n", false, buywindow)
guiMemoSetReadOnly(memo, true)
edit8 = guiCreateEdit(388, 250, 153, 33, "", false, buywindow)
guiEditSetReadOnly(edit8, true)
buyButton = guiCreateButton(388, 209, 156, 31, "Buy", false, buywindow)
guiSetProperty(buyButton, "NormalTextColour", "FFAAAAAA")
label1 = guiCreateLabel(16, 29, 164, 19, "Weed (speed and Low gravity)", false, buywindow)
guiLabelSetColor(label1, 251, 213, 3)
label2 = guiCreateLabel(248, 29, 80, 15, "God (Health 200)", false, buywindow)
guiLabelSetColor(label2, 0, 253, 5)
label3 = guiCreateLabel(427, 28, 98, 16, "Speed (Speed X10)", false, buywindow)
guiLabelSetColor(label3, 250, 2, 194)
label4 = guiCreateLabel(58, 107, 70, 15, "Lsd (Hallucination)", false, buywindow)
guiLabelSetColor(label4, 0, 248, 251)
label5 = guiCreateLabel(216, 103, 145, 19, "Steroids (Doubling the enemy injury)", false, buywindow)
guiLabelSetColor(label5, 2, 248, 60)
label6 = guiCreateLabel(423, 103, 108, 18, "Heroin (Semi-immune)", false, buywindow)
guiLabelSetColor(label6, 201, 48, 196)
label7 = guiCreateLabel(33, 173, 120, 18, "buy all types of drugs", false, buywindow)
guiLabelSetColor(label7, 255, 0, 0)
closeButton = guiCreateButton(19, 250, 139, 44, "Close", false, buywindow)
    ...
    if (guiGetVisible(buywindow) == true) then
        ...
        if tonumber(guiGetText(edit5)) ~= nil then
            setElementData(
                localPlayer,
                "SteroidsMoney",
                guiGetText(edit5) * getElementData(localPlayer, "SteroidsPrice")
            )
        else
            guiSetText(edit5, "0")
        end
        ...
        if tonumber(guiGetText(edit7)) ~= nil then
            local alljm3 =
                getElementData(localPlayer, "HeroinPrice") + getElementData(localPlayer, "SteroidsPrice") +
                getElementData(localPlayer, "LsdPrice") +
                getElementData(localPlayer, "SpeedPrice") +
                getElementData(localPlayer, "GodPrice") +
                getElementData(localPlayer, "WeedPrice")
...
setTimer(
    function()
        if (guiGetVisible(buywindow) == true) then
            guiSetText(
                edit8,
                getElementData(localPlayer, "WeedMoney") + getElementData(localPlayer, "GodMoney") +
                    getElementData(localPlayer, "SpeedMoney") +
                    getElementData(localPlayer, "LsdMoney") +
                    getElementData(localPlayer, "SteroidsMoney") +
                    getElementData(localPlayer, "HeroinMoney") +
                    getElementData(localPlayer, "ALLMoney")
            )
        end
    end,
    50,
...
addEventHandler(
    ...
    function()
        ...
        elseif (source == buyButton) then
            if
                (tonumber(guiGetText(edit1)) > 0) or (tonumber(guiGetText(edit2)) > 0) or
                    (tonumber(guiGetText(edit3)) > 0) or
                    (tonumber(guiGetText(edit4)) > 0) or
                    (tonumber(guiGetText(edit5)) > 0) or
                    (tonumber(guiGetText(edit6)) > 0) or
                    (tonumber(guiGetText(edit7)) > 0)
             then
                ...
                outputChatBox("Done buy drugs", 255, 255, 0, true)
            end
        end
    end
)
...
radioButton1 = guiCreateRadioButton(17, 27, 205, 21, "Bareta", false, useWindow)
...
radioButton2 = guiCreateRadioButton(17, 58, 205, 21, "Popper (Vida al 200)", false, useWindow)
...
radioButton3 = guiCreateRadioButton(17, 89, 205, 21, "Perica", false, useWindow)
...
radioButton4 = guiCreateRadioButton(17, 120, 205, 21, "Lsd", false, useWindow)
...
radioButton5 = guiCreateRadioButton(17, 151, 205, 21, "Bazuko (Doble daño)", false, useWindow)
...
radioButton6 = guiCreateRadioButton(17, 182, 205, 21, "Gale (Semi-inmune)", false, useWindow)
guiSetProperty(radioButton6, "HoverTextColour", "FF660066")
guiSetProperty(radioButton6, "NormalTextColour", "FFD425BE")
WeedLabel = guiCreateLabel(228, 28, 24, 20, "" .. tostring(getElementData(localPlayer, "Weed")) .. "", false, useWindow)
GodLabel = guiCreateLabel(228, 59, 24, 20, "" .. tostring(getElementData(localPlayer, "God")) .. "", false, useWindow)
SpeedLabel =
    guiCreateLabel(228, 89, 24, 20, "" .. tostring(getElementData(localPlayer, "Speed")) .. "", false, useWindow)
LsdLabel = guiCreateLabel(228, 120, 24, 20, "" .. tostring(getElementData(localPlayer, "Lsd")) .. "", false, useWindow)
SteroidsLabel =
    guiCreateLabel(228, 151, 24, 20, "" .. tostring(getElementData(localPlayer, "Steroids")) .. "", false, useWindow)
Heroin = guiCreateLabel(228, 182, 24, 20, "" .. tostring(getElementData(localPlayer, "Heroin")) .. "", false, useWindow)
...
addEvent("onClientUseDrug", true)
addEventHandler("onClientUseDrug",localPlayer, updatePanel)
------- Events
