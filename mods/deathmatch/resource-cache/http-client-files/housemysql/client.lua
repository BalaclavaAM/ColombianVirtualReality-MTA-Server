...
local houseInteriors = { -- I don't really use tables
	[1] = "223.0538482666, 1287.3552246094, 1082.140625, 1, Shitty house 1", -- Bien
	[2] = "2233.6594238281, -1114.9837646484, 1050.8828125, 5, Hotel room 1", -- Bien
--	[3] = "2365.22265625, -1135.5612792969, 1050.8825683594, 8, Modern house 1", -- pelle
	[3] = "2282.9401855469, -1140.1246337891, 1050.8984375, 11, Hotel room 2", -- Bien
	[4] = "2196.7075195313, -1204.3569335938, 1049.0234375, 6, Modern house 2", -- Bien
	[5] = "2270.0834960938, -1210.4854736328, 1047.5625, 10, Modern house 3", -- Bien
	[6] = "2308.755859375, -1212.5498046875, 1049.0234375, 6, Shitty house 2", -- Bien
--	[8] = "2217.7729492188, -1076.2110595703, 1050.484375, 1, Hotel room 3", -- Pelle
	[7] = "2237.5009765625, -1080.9367675781, 1049.0234375, 2, Good house 1", -- Bien
--	[10] = "2317.7609863281, -1026.4268798828, 1050.2177734375, 9, Good Stair house 1", --Pelle
	[8] = "261.05038452148, 1284.7646484375, 1080.2578125, 4, Good house 2", -- Bien
	[9] = "140.18000793457, 1366.7183837891, 1083.859375, 5, Modern house 4(Rich)", -- Bien
	[10] = "83.037002563477, 1322.6156005859, 1083.8662109375, 9, Good Stair house 2", -- Bien
	[11] = "-283.92623901367, 1471.0096435547, 1084.375, 15, Good Stair house 3", -- Bien
--	[15] = "-261.03778076172, 1456.6539306641, 1084.3671875, 4, Good Stair house 4", -- pelle
	[12] = "-42.609268188477, 1405.8033447266, 1084.4296875, 8, Shitty house 3", -- Bien
	[13] = "-68.839866638184, 1351.4775390625, 1080.2109375, 6, Shitty house 4", -- Bien
--	[18] = "2333.068359375, -1077.0648193359, 1049.0234375, 6, Shitty house 5", -- Pelle
	[14] = "1261.1168212891, -785.38037109375, 1091.90625, 5, Mad Doggs Mansion", -- Bien
--	[20] = "2215.1145019531, -1150.4993896484, 1025.796875, 15, Jefferson Motel", -- Pelle
--	[21] = "2352.4575195313, -1180.9454345703, 1027.9765625, 5, Burning desire house(Buggy)", --Pelle
	[15] = "421.94845581055, 2536.5021972656, 10, 10, Abandoned Tower", -- Bien
	[16] = "2495.9753417969, -1692.4174804688, 1014.7421875, 3, Johnson house", -- Bien
	[17] = "-2158.7209472656, 642.83074951172, 1052.375, 1, Bet Interior", -- Bien
	[18] = "1701.1682128906, -1667.759765625, 20.21875, 18, Atrium lobby", -- Bien
--	[26] = "2324.499, -1147.071, 1050.71, 12, Modern House 5(Rich)", --Pelle
	[19] = "244.411987,305.032989,999.148437, 1, 1 Room house",
	[20] = "271.884979,306.631988,999.148437, 2, 1 Room house",
	[21] = "302.180999,300.722991,999.148437, 4, 1 Room house",
	[22] = "-94.92, -644.42, 2022.5, 8, Custom House",
}
...
addEventHandler("onClientHouseSystemMenueUpdate", getRootElement(), function(string, value, int)
	...
	if(string == "LOCKED") then
		if(value == 0) then
			guiSetText(Knopf2[6], "Bloquear casa")
		else
			guiSetText(Knopf2[6], "Desbloquear casa")
		end
	elseif(string == "MONEY") then
		guiSetText(Label2[3], "Dinero actual:\n"..value.."$")
	...
	elseif(string == "RENTABLE") then
		if(value == true) then
			guiSetText(Knopf2[9], "Quitar de renta")
		else
			guiSetText(Knopf2[9], "Colocar en renta")
		end
	elseif(string == "RENTALPRICE") then
		guiSetText(Label2[15], "Precio de renta: $"..value)
...
addEventHandler("onClientHouseSystemInfoMenueOpen", gMe, function(owner, x, y, z, price, locked, rentable, rentalprice, id, doingrent, isrentedin, canadd)
	...
	local rentstate = "no"
	if(tonumber(rentable) == 1) then rentstate = "Si" end
	
	local X, Y, Width, Height = getMiddleGuiPosition(270,251)
	Fenster3[1] = guiCreateWindow(X, Y, Width, Height,"Informacion",false)
	Label3[1] = guiCreateLabel(12,43,253,15,"ID: "..id,false,Fenster3[1])
	...
	Label3[4] = guiCreateLabel(12,61,251,18,"Ubicacion: "..getZoneName(x, y, z, false),false,Fenster3[1])
	guiLabelSetHorizontalAlign(Label3[4],"center",false)
	guiSetFont(Label3[4],"default-bold-small")
	Label3[5] = guiCreateLabel(12,81,250,14,"Dueño: "..owner,false,Fenster3[1])
	...
	Label3[7] = guiCreateLabel(12,118,250,14,"En renta: "..rentstate,false,Fenster3[1])
	...
	Label3[9] = guiCreateLabel(12,155,247,12,"Bloqueada: "..lockedstate,false,Fenster3[1])
	...
	Knopf3[2] = guiCreateButton(137,175,124,32,"Rentar",false,Fenster3[1])
	...
	Knopf3[5] = guiCreateButton(220,141,50,32,"Inicio",false,Fenster3[1])
	...
	-- ENTER BUTTON --
	if(getElementData(gMe,"account") ~= owner) then
	...
	if(getElementData(gMe,"account") == owner) then
		...
		if(isrentedin == true) then
			...
			guiSetText(Knopf3[2], "Desarrendar")
		end
	end
	...
	if(owner ~= "no-one") then
		...
		if(owner == getElementData(gMe,"account")) then
	...
	addEventHandler("onClientGUIClick", Knopf3[5], function()
		triggerServerEvent("housemysql:SetSpawn", gMe, id)
		showCursor(false)
		destroyElement(Fenster3[1])
	end, false)
end)
...
addEventHandler("onClientHouseSystemMenueOpen", gMe, function(owner, locked, money, weap1, weap2, weap3, id, rentable, rent, tenands)
	if(locked == nil) then
		...
		Fenster2[1] = guiCreateWindow(X, Y, Width, Height, "Menu de casa",false)
		...
		Label2[3] = guiCreateLabel(17,40,106,32,"Dinero actual:\n0$",false,Fenster2[1])
		...
		Knopf2[4] = guiCreateButton(301,104,107,30,"Depositar arma",false,Fenster2[1])
		Knopf2[5] = guiCreateButton(301,142,107,30,"Tomar arma",false,Fenster2[1])
		...
		Knopf2[7] = guiCreateButton(424,88,114,27,"Colocar precio de renta",false,Fenster2[1])
		...
		guiGridListAddColumn(Grid2[1],"Inquilinos",0.8)
		Knopf2[8] = guiCreateButton(422,171,115,30,"Quitar inquilino",false,Fenster2[1])
		...
		Knopf2[9] = guiCreateButton(423,137,115,30,"Colocar en renta",false,Fenster2[1])
		-- SET THE SETTINGS --
		-- TENANDS --
		guiGridListClear(Grid2[1])
		...
		addEventHandler("onClientGUIClick", Knopf2[8], function()
			if(owner == false) then exports.NGMessages:sendClientMessage ("No eres el dueño de esta casa!", 255, 0, 0) return end
			local user = guiGridListGetItemText(Grid2[1], guiGridListGetSelectedItem(Grid2[1]), 1)
			if(user == "") or (user == " ") then return end
			if(guiCheckBoxGetSelected(Checkbox2[1]) == false) then exports.NGMessages:sendClientMessage ("Debes estar seguro!", 255, 0, 0) return end
		...
		addEventHandler("onClientGUIClick", Knopf2[7], function()
			...
			if(value == nil) or(value < 1) or (value > 20000) then exports.NGMessages:sendClientMessage ("Mal ingreso!", 255, 0, 0) return end
		...
		addEventHandler("onClientGUIClick", Knopf2[2], function()
			...
			if(value < 0)  then exports.NGMessages:sendClientMessage ("Ingreso incorrecto de datos!", 255, 0, 0) return end
			value = math.floor(value)
		...
		addEventHandler("onClientGUIClick", Knopf2[1], function()
			...
			if(value == "") then return end
			value = tonumber(value)
			if(value < 0) or (value > getPlayerMoney(gMe)-1) then exports.NGMessages:sendClientMessage ("Ingreso incorrecto de datos!", 255, 0, 0) return end
			value = math.floor(value)
...
addEventHandler("onClientHouseSystemGUIStart", gMe, function()
	...
	addEventHandler("onClientGUIClick", Knopf[3], function()
		...
		if(x == "") or (y == "") or (z == "") or (intx == "") or (inty == "") or (intz == "") then exports.NGMessages:sendClientMessage ("You must insert a value in all text fields!", 255, 0, 0) return end
		price = tonumber(price)
		if(price < 0) then exports.NGMessages:sendClientMessage ("Bad Price!", 255, 0, 0) return end
	...
	addEventHandler("onClientGUIClick", Knopf[1], function()
		...
		if(id == nil) then exports.NGMessages:sendClientMessage ("You must select a house interior!", 255, 0, 0) return end
