...
function onClientColShapeHit(thePlayer)
	if(thePlayer == getLocalPlayer()) then
		if(getElementData(source, "Casino")) then
			...
			if(dat[2] == "SLOT") then
				triggerEvent("ToolTip", localPlayer, "Jugar en las tragaperras cuesta #457C3B$"..dat[4].."#FFFFFF\nPresiona Enter Para jugar")
				bindKey("enter", "down", PlayCasino) 
				ActionCasinoMarker = source
			elseif(dat[2] == "Roulette") then
				triggerEvent("ToolTip", localPlayer, Text("Max Wager").." #457C3B$"..dat[4].."#FFFFFF\nPresiona Enter para jugar, wasd para desplazarte, enter para apostar, space para girar")
				bindKey("enter", "down", PlayCasino) 
				ActionCasinoMarker = source
			end
		end
