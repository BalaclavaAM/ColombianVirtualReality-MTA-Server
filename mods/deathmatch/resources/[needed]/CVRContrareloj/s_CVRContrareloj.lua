exports.NGSQL:db_exec([[CREATE TABLE IF NOT EXISTS racedata ( Username VARCHAR(200), besttime INT, checkpointstatistics TEXT, race VARCHAR(20), club VARCHAR(20) )]]);
...
function endMinigameWin(player,time,besttime,racename,tablecheck)
    if getPedOccupiedVehicle(player) then
        if getPedOccupiedVehicle(player) == cars[player] then
            if time<besttime then
                ...
                updateGreatestTime(time,accname,racename,club,tablecheck)
            ...
            exports['NGJobs']:updateJobColumn(getAccountName(getPlayerAccount(player)), "DriftPoints", "AddOne")
...
function uploadNewTableSpeed(newtable,racename)
    local accname=getAccountName(getPlayerAccount(source))
    return exports['NGSQL']:db_exec ( "UPDATE `racedata` SET `checkpointstatistics`=? WHERE Username=? and race=?",toJSON(newtable),accname,racename )
end
addEvent("CVRContrareloj:UploadTable",true)
addEventHandler("CVRContrareloj:UploadTable",root,uploadNewTableSpeed)

function updateGreatestTime(time,accname,racename,club,tablecheck)
    return exports['NGSQL']:db_exec ( "UPDATE `racedata` SET `besttime`=?, `club`=?, `checkpointstatistics`=? WHERE Username=? and race=?",time,club,tablecheck,accname,racename )
end
...
function startMinigame(player,race)
    if getElementData(player,"Job")=="Drifter" then
        ...
        if ( type ( data ) ~= "table" or #data < 1 ) then
            exports['NGSQL']:db_exec ( "INSERT INTO racedata ( `Username`, `besttime`, `checkpointstatistics`, `race`, `club`) VALUES ( ?,?,?,?,? );", getAccountName ( acc ),99999999999,toJSON({}),race,elclub)
        end
        local data=exports['NGSQL']:db_query ( "SELECT besttime,checkpointstatistics FROM racedata WHERE Username=? and race=?", getAccountName ( acc ),race )
        local greatestTime = data[1]['besttime']
        local checkpointTable = data[1]['checkpointstatistics']
        triggerClientEvent(player,"sendGreatestTime",player,greatestTime,checkpointTable)
        ...
        setTimer(function(carro)
            ...
            triggerClientEvent(player,"CreateClientCheckpointsContrareloj",player,race)
