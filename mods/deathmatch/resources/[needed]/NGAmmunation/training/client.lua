...
addEventHandler ( "NGAmmunation:Module->Training:onClientHitMarker", root, function ( s )
	...
	if not ( d[allowedWeapons[w]] ) then 
		return exports.NGMessages:sendClientMessage ( "No puedes entrenar con esta arma", 255, 255, 0 )
	end
	if ( d[allowedWeapons[w]] > 90 ) then
		return exports.NGMessages:sendClientMessage ( "Esta arma ya está al máximo!", 255, 255, 0 )
	end
...
function Training_OnRender ( )
	...
	dxDrawText ( "Aquí en Ammu-Nation, puedes subir de nivel con tus armas. Algunas armas (F3 -> Estadísticas) tienen un nivel de 0 a 100. Puedes de subir de nivel tus armas aqui para que disparen mas rápido o tener doble arma al tiempo.", windowX + ( rsx * 20 ), windowY + ( rsy * 40 ), ( windowX+windowW)-(rsx * 20 ), (windowY+windowH)-(rsy*50), tocolor ( 255, 255, 255, 255 ), rsy*1.1, "default-bold", "left", "top", false, true )
	dxDrawText ( "Arma seleccionada: N/A\nSkill: 0/100\nCosto: $1,200", windowX + ( rsx * 20 ), windowY + ( rsy * 225 ), ( windowX+windowW)-(rsx * 20 ), (windowY+windowH)-(rsy*280), tocolor ( 255, 200, 65, 255 ), rsy*1.1, "default-bold", "left", "top", false, true )
	dxDrawRectangle ( windowX+(rsx*20), info.rect.y, windowW-(rsx*40), rsy*35, tocolor ( 0, 0, 0, 150 ) )
	
	dxDrawText ( "Aceptar",  info.accept.x, info.accept.y, windowX+(windowW-(rsx*40)), info.accept.y+(rsy*35), tocolor(255,255,255,255), rsy*1.2, "default-bold", "center", "center" )
	dxDrawText ( "Salir",  info.exit.x, info.exit.y, windowX+(windowW-(rsx*40)), info.exit.y+(rsy*35), tocolor(255,255,255,255), rsy*1.2, "default-bold", "center", "center" )
	...
	if ( k == "space" ) then
		...
		elseif ( selectedOption == 0 ) then
			...
			if ( not allowedWeapons [ getPedWeapon ( localPlayer ) ] ) then
				return exports.NGMessages:sendClientMessage ( "Esta arma no está permitida para entrenar" )
			end
			
			if ( getPlayerMoney ( ) < 1200 ) then
				return exports.NGMessages:sendClientMessage ( "No tienes $1,200 para entrenar esta arma", 255, 255, 0 )
			end
...
function Training_UserStats ( )
	...
	dxDrawText ( "Estado", winX, winY+(rsy*5), winX+winW, winY+winH, tocolor ( 255, 255, 255, 255 ), rsy*2, "default-bold", "center" )
	dxDrawText ( "Asesinados: "..tostring(training.TargetShot), winX+(rsx*25), winY+(rsy*50), 0, 0, tocolor ( 255, 160, 0, 255 ), rsy*1.5, "default-bold" )
	dxDrawText ( "Faltantes: "..tostring(training.RequiredPeds), winX+(rsx*25), winY+(rsy*90), 0, 0, tocolor ( 255, 160, 0, 255 ), rsy*1.5, "default-bold" )
		...
		if ( training.TargetShot == training.RequiredPeds ) then
			...
			setTimer ( function ( )
				...
				if ( trainingWeapon ~= getPedWeapon ( localPlayer ) ) then
					return exports.NGMessages:sendClientMessage ( "Cambiaste de arma.... No fue aumentado tu nivel.", 255, 0, 0 )
				end
				...
				exports.NGMessages:sendClientMessage ( "Subiste de Nivel esta arma, Nivel actual:: "..d[allowedWeapons[w]].."/100", 0, 255, 0 )
