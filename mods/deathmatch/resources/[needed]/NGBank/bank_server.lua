local bankLocations = {
	...
	{ 2157.46, 1599.4, 1006.19, 1, 0, {2157.46, 1599.4, 1006.19}},
	{ 822.42, 2.31, 1004.18, 3, 1, {1833.5, -1842.58, 13.58}}, --Buseteros
	{ 822.42, 2.31, 1004.18, 3, 2, {-2703.94, 639.37, 14.45}}, --SF
...
function sacarplata ( player, amount )
	local acc = getPlayerAccount ( player )
	if ( isGuestAccount ( acc ) ) then return false end
	local acc = getAccountName ( acc )
	if ( not doesBankAccountExist ( acc ) ) then
	...
	local today = exports.NGPlayerFunctions:getToday ( )
	local lg = getPlayerName ( player ).." sacó $"..amount
	local serial = getPlayerSerial ( player )
	local ip = getPlayerIP ( player )
	exports.NGSQL:db_exec ( "INSERT INTO bank_transactions ( account, log, serial, ip, thetime ) VALUES ( ?, ?, ?, ?, ? )",
		acc, lg, serial, ip, today )
...
function meterplata ( player, ammount )
	local acc = getPlayerAccount ( player )
	...
	accounts[acc] = accounts[acc] + ammount
	return true
end 
