...
function desbanearPersona ( p,_,p2 )
	if isObjectInACLGroup("user."..getAccountName(getPlayerAccount(p)),aclGetGroup("Level 5")) then
		local desbaneo = unbanAccount ( p2 )
		if desbaneo then
			outputChatBox("Desbaneaste al chamo "..p2,p,255,255,0,true)
		else
			outputChatBox("El chamo "..p2.." no está baneado o esa no es la cuenta",p,255,255,0,true)
		end
	end
end 
addCommandHandler("desbanear",desbanearPersona,false,false)
