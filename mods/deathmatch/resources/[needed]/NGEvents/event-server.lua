...
function EventCore.StartEvent ( id )
	...
	local text = textCreateTextItem ( "El evento comienza en: 60", 0.6, 0.65, "hight", 255, 255, 0, 255, 2, "right", "bottom", 2 ) 
	...
	eventInfo = { 
		...
		interior						= events[id].interior or 0,
		nitro							= events[id].nitro or false, 
		author							= events[id].author or "CVR",
	}
	...
	DummyTimer1 = setTimer ( function ( )
		...
		textItemSetText ( eventInfo.dispText, "El evento comienza en : "..math.floor(getTimerDetails(eventInfo.timer)/1000) )
	end, 1000, 0 )
	...
	exports.NGMessages:sendClientMessage ( "Evento: "..tostring(events[id].name).." by:"..eventInfo.author.." ha sido creado! comienza en 60 segundos, use /joinevent para unirse!", root, 0, 255, 0 )
	outputChatBox("Evento:"..tostring(events[id].name).." by:"..eventInfo.author.." ha sido creado! comienza en 60 segundos, use /joinevent para unirse!", root, 0,255,0, true)
...
function EventCore.PlayerJoinEventCommand ( p )
	if ( playersInEvent [ p ] ) then
		return exports.NGMessages:sendClientMessage ( "Ya estas en el evento", p, 0, 255, 255 )
	end
	
	local rSlots = eventInfo.maxSlots-table.len(playersInEvent)
	if ( getElementData ( p, "NGPolice:JailTime" ) ) then return exports.NGMessages:sendClientMessage ( "No puedes unirte al evento estando en la carcel", p, 255, 0, 0 ) end
	if ( isPedInVehicle ( p ) ) then return exports.NGMessages:sendClientMessage ( "Sal del vehiculo primero", p, 255, 255, 0 ) end
	if ( getPlayerWantedLevel ( p ) > 0 and EventCore.GetDistanceToNearestCop ( p ) <= 170 ) then return exports.NGMessages:sendClientMessage ( "Tienes estrellas y hay un policia cerca.", p, 255, 255, 0 ) end
	if ( getElementInterior ( p ) ~= 0 ) then return exports.NGMessages:sendClientMessage ( "Ve afuera para ir al evento", p, 255, 255, 0 ) end
	if ( rSlots <= 0 ) then return exports.NGMessages:sendClientMessage ( "Evento lleno!", p, 255, 0, 0 ) end
	if ( getPlayerMoney(p) < cost ) then return exports.NGMessages:sendClientMessage ( "No tienes suficiente dinero para participar del evento!", p, 255, 0, 0 ) end
	...
	--takeAllWeapons ( p )
	takePlayerMoney(p, cost)
	if ( rSlots > 0 ) then exports.NGMessages:sendClientMessage ( "Evento: "..getPlayerName(p).." se ha unido. Solo "..rSlots.." cupos quedan", root, 255, 255, 2550 )
	else exports.NGMessages:sendClientMessage ( "Evento: "..getPlayerName(p).." se ha unido. El evento ahora está lleno", root, 255, 0, 0 ) end
	...
	if ( eventInfo.onlyOnePersonPerWarp ) then
		...
		--end
		x, y, z, r = unpack ( table.remove(warps,1) )
	end
	...
	if ( eventInfo.vehicleID ) then
		...
		setElementInterior( eventObjects.playerItems[p], eventInfo.interior)
		warpPedIntoVehicle ( p, eventObjects.playerItems[p] )
		if eventInfo.nitro then addVehicleUpgrade(eventObjects.playerItems[p], 1010) end
		setCameraTarget ( p, p ) 
		...
		setElementData(p,"canDriveby", false)
...
function EventCore.BeginTheEvent ( )
	...
	if ( #EventCore.GetPlayersInEvent ( ) < eventInfo.minSlots ) then
		EventCore.EndEvent ("noplayers")
		return exports.NGMessages:sendClientMessage ( "El evento se ha cancelado por no haber suficientes jugadores.", root, 255, 255, 0 )
	end
	
	exports.NGMessages:sendClientMessage ( "Evento: Que comience el juego!", root, 0, 255, 0 )
	...
	for i, p in pairs ( EventCore.GetPlayersInEvent ( ) ) do
		...
		setElementData(p,"canDriveby", not eventInfo.disableWeapons)
...
function EventCore.GetPlayersInEvent ( )
	local c = { }
	if playersInEvent == nil then return c end
	for i, v in pairs ( playersInEvent ) do
		if ( isElement ( i ) ) then
			table.insert ( c, i )
		else
...
function EventCore.RemovePlayerByWasted ( )
	EventCore.RemovePlayerFromEvent ( source, true )
	exports.NGMessages:sendClientMessage ( getPlayerName ( source ).." ha muerto en el evento!", root, 255, 255, 0 )
	setElementData(source,"canDriveby",true)
...
function EventCore.RemovePlayerByQuit ( )
	EventCore.RemovePlayerFromEvent ( source, true )
	exports.NGMessages:sendClientMessage ( getPlayerName ( source ).." se ha desconectado!", root, 255, 255, 0 )
	local c = EventCore.GetPlayersInEvent()
	if table.len(c) == 0 then
		EventCore.EndEvent ( )
	end
...
function EventCore.EndEvent (reason )
	reason = reason or 0
	for i, v in ipairs ( EventCore.GetPlayersInEvent ( ) ) do
		if (reason == "noplayers") then
			givePlayerMoney(v,cost)
			setElementData(v,"canDriveby",true)
		end
...
function EventCore.RemovePlayerFromEvent ( p, reset )
	if playersInEvent == nil then return end 
...
function isPlayerInEvent ( p )
	if playersInEvent == nil then return false end 
	if playersInEvent [ p ] then
		return true
	else
		return false
...
function EventCore.WinPlayerEvent ( p )
	if ( isElement ( p ) ) then
		...
		if playersInEvent == nil then return end 
		local a = math.random ( cost, cost+500 ) * eventInfo.originalPlayerCount
		if a == 0 then return end 
		exports.NGMessages:sendClientMessage ( "Evento: "..getPlayerName(p).." gano $"..a.." por ganar el "..eventInfo.name.." evento!", root, 0, 255, 0 )
		--exports.NGLogs:outputServerLog ( "Event: "..getPlayerName(p).." has won the "..eventInfo.name.." event" )
		givePlayerMoney ( p, a )
		local user = getAccountName(getPlayerAccount(p))
		local col = "events"
...
addCommandHandler ( "leaveevent", function ( p )
	if ( isPlayerInEvent ( p ) ) then
		...
		setElementData(p,"canDriveby",true)
