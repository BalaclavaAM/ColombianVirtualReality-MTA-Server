function glue()
	...
	if not getPedOccupiedVehicle(player) then
		...
		local maletin = getElementData(player,"bag") or false
		if vehicle and getElementType(vehicle) == "vehicle" and not(maletin) and not(tazeao) and not(tomboide) then
			...
			addEventHandler("onClientPlayerVehicleEnter", localPlayer, unglue)
		end
	end
end
...
function unglue ()
	...
	bindKey("x","down",glue)
	removeEventHandler("onClientPlayerVehicleEnter", localPlayer, unglue)
end
