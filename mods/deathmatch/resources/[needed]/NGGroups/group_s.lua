...
addEventHandler ( "NGGroups->gEvents:onPlayerDeleteGroup", root, function ( group )
	if getElementData(source,"Group Rank") == "Founder" then
		deleteGroup ( group )
		...
		outputChatBox("Ermano q pena ud no puede borrar grupos",source,255,0,0)
	end
end )
...
function getPlayerGroupRank ( player )
	local g = getElementData ( player, "Group Rank" ) or "None"
	if ( g:lower ( ) == "none" ) then
		g= nil
	end
	return g
...
function canRankInvite ( player )
	if ( getPlayerGroup( player )) then
		if ( getPlayerGroupRank( player )) then
			local gr = getPlayerGroup( player )
			local grr = getPlayerGroupRank ( player )
			if groups[gr]['ranks'][grr]["member_invite"] then
				return true
			end
		end
	end
...
function enviarPeticion( p, _, p2, ... )
	if getElementData(p,"Group") then
		if canRankInvite ( p ) then
			if p2 then
				local toJ = getPlayerFromName ( p2 ) or exports['NGPlayerFunctions']:getPlayerFromNamePart ( p2 )
				if toJ then
					if toJ~=p then
						if exports['NGPlayerFunctions']:getPlayerPlaytime ( p ) >59 then
							if (getElementData(toJ,"Group")=="None") then
								if not (isGuestAccount(getPlayerAccount(toJ))) then
									if not(petitions[toJ]) then
										petitions[toJ]=p
										outputGroupLog ( group, "Invited "..getPlayerName(toJ).." ("..getAccountName(getPlayerAccount(toJ))..")", source )
										local r, g, b = getGroupColor ( group )
										outputChatBox(prefix.."Invitaste a #00F5F5"..getPlayerName(toJ).."#FFFFFF a tu gang!.",p,255,232,0,true)
										outputChatBox(prefix.."#00F5F5"..getPlayerName(p).."#FFFFFF te ha invitado a la gang #00F5F5"..getElementData(p,"Group").."#FFFFFF usa #34FF02/aceptargang #FFFFFF(en 20 sg la petición se #FF0202eliminará#FFFFFF)",toJ,255,0,0,true)
										setTimer(function()
											petitions[toJ]=nil
										end, 60000, 0)
									else
									...
									outputChatBox(prefix.."Sólo puedes enviarle invitaciones a gente logeada.",p,255,0,0,true)
								end
							else
								outputChatBox(prefix.."Sólo puedes enviarle invitaciones a jugadores con 60 minutos de juego o más.",p,255,0,0,true)
							end
						else
							outputChatBox(prefix.."Este está en la gang #00F5F5"..getElementData(toJ,"Group").."#FFFFFF por lo tanto no puedes enviarle peticiones",p,255,0,0,true)
						end
					else
						outputChatBox( "Hermano cómo te vas a dar gang a tí mismo", p, 255, 0, 0,true)
					end
				else
					outputChatBox( "El usuario "..p2.." no existe o no está conectado", p, 255, 0, 0,true)
				end
			else
				outputChatBox( prefix.."La sintaxis del CMD es /invitargang [Usuario]", p, 255, 0, 0,true)
			end
		else
			outputChatBox( "Tienes que tener un rango con permisos para invitar a la gang", p, 255, 0, 0,true)
		end
	else
		outputChatBox( "Usted no posee gang", p, 255, 0, 0,true)
	end
end
addCommandHandler("invitargang",enviarPeticion)

function recibirPeticion( p )
	if petitions[p] then
		if (getElementData(p,"Group")=="None") then
			if isElement(petitions[p]) then
				local queryclub=getElementData(petitions[p],"Group") or "None"
				if queryclub ~= "None" then
					setPlayerGroup(p,queryclub)	
					outputGroupMessage ( getPlayerName ( p ).." se unió al grupo!", queryclub )
					outputChatBox(prefix.."#00F5F5"..getPlayerName(p).."#FFFFFF se ha unido a la gang",petitions[p],255,0,0,true)
					outputChatBox(prefix.."Te has unido a la gang #00F5F5"..getElementData(p,"Group").."#FFFFFF!",p,255,0,0,true)
				end
			else
			...
			outputChatBox(prefix.."Ya te uniste a un grupo!.",p,255,232,0,true)
		end
	else
		outputChatBox(prefix.."No tienes peticiones pendientes, pide que te la reenvíen!.",p,255,232,0,true)
	end
end
addCommandHandler("aceptargang",recibirPeticion)
...
function enableGC(p)
	...
	if isObjectInACLGroup("user."..accName, aclGetGroup ( "Level 5" ) ) then
		if getElementData(p,"ReadGC") then
			removeElementData(p,"ReadGC")
		else
			setElementData(p,"ReadGC",true)
		end
	end
end
addCommandHandler("vergc",enableGC)

function outputGroupMessage ( message, group, blockTag, persona )
	...
	if persona then
		for i, v in ipairs ( getElementsByType ( "player" ) ) do
			if ((tostring(getElementData ( v, "Group" ) ):lower ( ) == group:lower()) or getElementData(v,"ReadGC")) and not(exports['NGChat']:isIgnoredBy(persona,v)) then
				outputChatBox ( message, v, r, g, b, true )
			end 
		end
	else
		for i, v in ipairs ( getElementsByType ( "player" ) ) do
			if ((tostring(getElementData ( v, "Group" ) ):lower ( ) == group:lower()) or getElementData(v,"ReadGC")) then
				outputChatBox ( message, v, r, g, b, true )
			end 
		end
	end 
...
addCommandHandler ( "gc", function ( plr, _, ... )
	...
	if not antispammer[plr] or (getTickCount()-antispammer[plr]>1999) then
		outputGroupMessage("[Group] ".. exports.ngchat:getChatLine ( plr, message ), g, true, plr )
		antispammer[plr]=getTickCount()
	else
		return exports.ngmessages:sendClientMessage ( "Solo puedes enviar mensajes de grupo cada 2 sg", plr, 255, 0, 0 )
	end
end )



function dargang(p,_,player,...)
	accountname = getAccountName (getPlayerAccount(p))
	if isObjectInACLGroup ( "user." .. accountname, aclGetGroup ( "Admin" ) ) or isObjectInACLGroup ( "user." .. accountname, aclGetGroup ( "Level 4" ) ) or isObjectInACLGroup ( "user." .. accountname, aclGetGroup ( "Level 5" ) ) then
		local p2 = getPlayerFromName(player) or exports['NGPlayerFunctions']:getPlayerFromNamePart ( player )
		if p2 then
			local gang=table.concat ( { ... }, " " )
			if gang then
				if doesGroupExist(gang) then
					setElementData ( p2, "Group", gang )
					setElementData ( p2, "Group Rank", "Member")
					outputChatBox("Gang asignada correctamente",p,255,255,0)
				else
					outputChatBox("La gang "..gang.." no existe",p,255,0,0)
				end
			else
				outputChatBox("Sintaxis: /dargang [jugador] [gang]",p,255,0,0)
			end
		else
			outputChatBox("Sintaxis: /dargang [jugador] [gang]",p,255,0,0)
		end
	else
		outputChatBox("Debes ser admin lvl 4 o más para utilizar este comando",p,255,0,0)
	end
end
addCommandHandler("dargang",dargang)


function borrarGruposPequenhos(p)
	if isObjectInACLGroup("user."..getAccountName(getPlayerAccount(p)),aclGetGroup("Level 5")) then
		local borrados = 0
		for groupname, info in pairs(groups) do
			if table.len(info['members'])<3 then
				borrados = borrados +1
				deleteGroup(groupname)
			end
		end
		outputChatBox("Borramos "..borrados.." grupos!",p,255,0,0,true)
	end
end
addCommandHandler("borrargrupos",borrarGruposPequenhos)