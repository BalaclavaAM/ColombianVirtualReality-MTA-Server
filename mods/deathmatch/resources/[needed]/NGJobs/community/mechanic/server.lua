...
addEventHandler ( "NGJobs:Mechanic_AttemptFixVehicle", root, function ( v )
	...
	local price = (30000-(30*math.floor(getElementHealth(v))))
	--
	if ( getPlayerMoney ( controller ) >= price ) then
		if ( price == 0 ) then
			triggerClientEvent ( source, "NGJobs:Mechanic_CancelFixingRequest", source, 'source' )
...
addEventHandler ( "NGJobs:Mechanic_onVehicleCompleteFinish", root, function ( p )
	...
	if ( isElement ( p ) ) then
		...
		if ( isElement ( car ) ) then
			local price = (30000-(30*math.floor(getElementHealth(car))))
			--(1000-(math.floor(getElementHealth(car))))
			fixVehicle ( car )
