...
--настройки такси 421 ид
setModelHandling(421, "maxVelocity", 150) --Макс скорость
setModelHandling(421, "engineAcceleration", 13) --Ускарение
setModelHandling(421, "engineInertia", 20) --Инерция
--настройки такси 421 ид
setModelHandling(555, "maxVelocity", 150) --Макс скорость
setModelHandling(555, "engineAcceleration", 13) --Ускарение
setModelHandling(555, "engineInertia", 20) --Инерция
--
...
local passagir = getElementData(player,"passagir_Taxi_value")
	...
	elseif tonumber(passagir) == 100 then
		vehTaxi[player] = createVehicle (438,x, y, z,0,0,-90)
	end
...
if isElement(markers[player]) then
	...
	if isPedInVehicle(player) then
		...
		if (getElementModel(vehH) == 420) or (getElementModel(vehH) == 438) then
...
if isElement(markers[player]) then
	...
	local vehH = getPedOccupiedVehicle ( player )
		if (getElementModel(vehH) == 420) or (getElementModel(vehH) == 438) then
			...
			local money = money*moneyX*1.5
			givePlayerMoney(player,money)
			--Base de datos
			updateJobColumn ( getAccountName ( getPlayerAccount (player) ), "carreras", "AddOne" )
			--
...
addEvent ( "NGJobs->Module->Job->taxi->OnClientRequestF5Data", true )
addEventHandler ( "NGJobs->Module->Job->taxi->OnClientRequestF5Data", root, function ( )
	local data = { }
	local carreras = getJobColumnData ( getAccountName ( getPlayerAccount ( source ) ), getDatabaseColumnTypeFromJob ( "taxista" ) )
	data.carreras = carreras or 0
	data.nextRank = "None"
	data.requiredCarreras = 0
	data.rank = getElementData ( source, "Job Rank" ) or jobRanks [ 'taxista'] [ 0 ];
	...
	for i, v in pairs ( foreachinorder ( jobRanks [ 'taxista'] ) ) do 
		if ( isNext ) then 
			data.nextRank = v[2];
			data.requiredCarreras = v[1] - data.carreras;
			isNext = false;
			break;
		end 
	...
	triggerClientEvent ( source, "NGJobs->Module->Job->taxi->onServerSendClientJobInfo", source, data );
