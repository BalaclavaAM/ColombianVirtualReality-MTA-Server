...
function makeCriminalTheftVehicle ( )
	if getPlayerCount() >= 10 then
		...
		outputChatBox("                    #00ff00|VEHICULO CRIMINAL|                  ",root,0,0,255,true)
		outputChatBox("* #00ff00Un carro ha aparecido, robalo y llevalo a su destino ",root,0,0,255,true)
		outputChatBox("           #00ff00 te pagaran por el #00ff00(#ffffff Icono #ffff00Circulo Amarillo#00ff00)    ",root,0,0,255,true)
...
addEventHandler ( "NGJobs:Criminal:Theft:onPlayerCaptureVehicle", root, function ( )
	...
	local cash = math.random ( 40000, 70000 )
	...
	CriminalVehicleTheftTimer = setTimer ( makeCriminalTheftVehicle, 1000*math.random ( 600, 900 ), 1 )
	exports['NGLogs']:outputActionLog ( getPlayerName ( source ).." captured the criminal theft vehicle" )
	giveWantedPoints ( source, math.random ( 100, 300 ) ) 
end )

function loCreoONokas()
	if getPlayerCount() >= 10 then
		setTimer ( function ( )
		...
		setTimer(loCreoONokas,300000,1)
	end
end
addEventHandler("onResourceStart",resourceRoot,loCreoONokas)


--[[function carroCriminal ()
	makeCriminalTheftVehicle ( )
	CriminalVehicleTheftTimer = setTimer ( makeCriminalTheftVehicle, 300000, 0 )
end
addEventHandler ( "onResourceStart", resourceRoot, carroCriminal )]]