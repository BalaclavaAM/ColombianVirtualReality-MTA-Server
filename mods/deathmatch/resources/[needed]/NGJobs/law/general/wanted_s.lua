...
addEventHandler ( "onPlayerDamage", root, function ( p, weap, loss )
	...
	if ( isElement ( p ) and p ~= source and getElementType(p) ~= "object" ) then
		if getElementData(p,"isinCapitan") then return end
		...
		if t then
			...
			if weap then
				...
				elseif ( exports['NGPlayerFunctions']:isTeamLaw ( t ) and getPlayerWantedLevel ( source ) > 0 ) then
					return
				end
			end
		end
...
setTimer ( function ( )
	...
	for i, v in ipairs ( getElementsByType ( 'player' ) ) do 
		...
		local pasivo = ""
		if getElementData(v,"isPassive") then
			pasivo="[MP]"
		end
		if ( getPlayerWantedLevel ( v ) > 0 ) then
			setPlayerNametagText ( v, tostring ( getPlayerName ( v ) ).."["..tostring ( getPlayerWantedLevel ( v ) ).."]"..pasivo )
		else
			setPlayerNametagText ( v, tostring ( getPlayerName ( v ) )..pasivo )
		end
