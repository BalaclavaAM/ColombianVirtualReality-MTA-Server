...
addEventHandler("onResourceStart", resourceRoot, function()
	for i = 1, #availableTuningMarkers do
		...
		if currentTuning then
			...
			local blipMarker = createBlip ( currentTuning[1], currentTuning[2], currentTuning[3] , 27, 2, 0, 0, 0, 255, 0, 6550, root)
...
addEventHandler("tuning->OpticalUpgrade", root, function(vehicle, type, upgrade)
	if vehicle and getElementData ( vehicle, "NGVehicles:VehicleAccountOwner") and getElementData ( vehicle, "NGVehicles:VehicleAccountOwner")==getAccountName(getPlayerAccount(source)) then
		if upgrade then
			if type == "add" then
...
function obtenerHandling(modelo)
	local hand = exports['NGVehicles']:getVehicleHandlingg(modelo)
	if hand then
		return hand
	end
	return outputChatBox("Error, intenta nuevamente",255,0,0)
end
	...
	if vehicle and getElementData ( vehicle, "NGVehicles:VehicleAccountOwner") and getElementData ( vehicle, "NGVehicles:VehicleAccountOwner")==getAccountName(getPlayerAccount(source)) then
		if data then
			...
			if not data[1][2] then -- Default upgrade
				...
				for _, property in ipairs(data) do
					local defaultHandling = obtenerHandling(vehicleModel)[property[1]]
	...
	if vehicle and getElementData ( vehicle, "NGVehicles:VehicleAccountOwner") and getElementData ( vehicle, "NGVehicles:VehicleAccountOwner")==getAccountName(getPlayerAccount(source)) then
		if property then
			if value then
				...
				setVehicleHandling(vehicle, property, obtenerHandling(getElementModel(vehicle))[property], false)
			end
...
addEventHandler("tuning->LicensePlate", root, function(vehicle, text)
	if vehicle and getElementData ( vehicle, "NGVehicles:VehicleAccountOwner") and getElementData ( vehicle, "NGVehicles:VehicleAccountOwner")==getAccountName(getPlayerAccount(source)) and text then
...
function takeServerMoney(ammount)
	if client then
		takePlayerMoney(client, tonumber(ammount))
	end
end
addEvent( "takeServerMoney", true ) 
addEventHandler("takeServerMoney", root , takeServerMoney)
