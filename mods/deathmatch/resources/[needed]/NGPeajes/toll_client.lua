...
-- Toll GUI
local tollGUI = guiCreateWindow(0.37, 0.22, 0.34, 0.41,"Peaje INVIAS",true)
...
local tollinfo = guiCreateLabel(0.34, 0.58, 0.42, 0.04,"Precio del peaje: $",true,tollGUI)
local tollinfoPrice = guiCreateLabel(0.53, 0.58, 0.42, 0.04,"",true,tollGUI)
local tollPay = guiCreateButton(0.17, 0.66, 0.64, 0.14,"Pagar",true,tollGUI)
local tollImpulses = guiCreateButton(0.17, 0.82, 0.64, 0.14,"Comprar pases",true,tollGUI)
local tollClose = guiCreateButton(0.88, 0.08, 0.09, 0.09,"X",true,tollGUI)

-- Impulses GUI
local impulsesGUI = guiCreateWindow(0.31, 0.24, 0.35, 0.22, "Comprar Pases", true)
...
local impulsesAdd = guiCreateButton(0.18, 0.21, 0.65, 0.20, "Agregar uno", true, impulsesGUI)
local impulsesCurrent = guiCreateMemo(0.39, 0.49, 0.16, 0.20, "0", true, impulsesGUI)
guiMemoSetReadOnly(impulsesCurrent, true)
local impulsesInfo = guiCreateLabel(0.25, 0.53, 0.13, 0.11, "Actuales:", true, impulsesGUI)
local impulsesRemove = guiCreateButton(0.19, 0.72, 0.65, 0.20, "Remover uno", true, impulsesGUI)
	...
	function (state, tollPrice, guest)
		...
		if state == 1 then
			if not guiGetVisible (tollGUI) then
				...
				guiSetText (impulsesAdd, "Agregar uno ($"..tollPrice..")")
