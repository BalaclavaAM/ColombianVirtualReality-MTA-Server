...
-- Settings
local tollPrice = 1000		-- Toll Price (Default: 150)
...
local tollbarrierLVLS2 = createObject (968, 1731.9, 507.64, 28.9, 0, 270, 338) 	-- gate LV > LS 2
				...
				if allowedTypes [getVehicleType (sourceVehicle)] then
					if isGuestAccount (pAcc) then
						if getPlayerMoney (hitElement) < tollPrice then
							exports.NGMessages:sendClientMessage ("[INVIAS] No tienes dinero suficiente. (Precio: $"..tollPrice..")", controller, 255, 0, 0)
						else
					...
					elseif not isGuestAccount (pAcc) then
						if getPlayerTeam (controller) and teamsImmuneToToll[getTeamName(getPlayerTeam(controller))] then
							...
							exports.NGMessages:sendClientMessage ("[INVIAS] Este pase ha sido financiado por Uribe.", controller, 120, 255, 0)
						else
							if getAccountData (pAcc, "toll.impulses") and getAccountData (pAcc, "toll.impulses") > 0 then
								...
								triggerEvent ("tollPaid", controller, true)
								exports.NGMessages:sendClientMessage ("[INVIAS] pase usado, restantes:#ffffff "..getAccountData (pAcc, "toll.impulses"), controller, 255, 120, 0)
					...
					exports.NGMessages:sendClientMessage ("[INVIAS] Este vehiculo no esta permitido", controller, 255, 0, 0)
				end
			else
				return
			end
...
function openRamp (freePass)
	...
	elseif isElementWithinMarker (source, tollboothLVLS2) and not isTimer (ob8) then
		moveObject (tollbarrierLVLS2, 1000, 1731.9, 507.64, 28.9, 0, 90, 0)
...
addEventHandler ("manageImpulses", root,
	function (value)
		...
		if value == 1 then
			...
			exports.NGMessages:sendClientMessage ("[INVIAS] Agregado un pase a tu cuenta, total: #ffffff"..getAccountData (acc, "toll.impulses"), source, 10, 230, 10)
			takePlayerMoney (source, tollPrice)
		elseif value == 2 then
			givePlayerMoney (source, tollPrice)
			setAccountData (acc, "toll.impulses", getAccountData (acc, "toll.impulses") - 1)
			exports.NGMessages:sendClientMessage ("[INVIAS] Removido un pase, tienes el dinero de vuelta. Restantes: #ffffff"..getAccountData (acc, "toll.impulses"), source, 255, 130, 0)
		end
	end
)
