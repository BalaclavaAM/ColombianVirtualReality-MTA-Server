...
local defaultSettings = {
    ...
    { "usersettings_usecustomhudc", "true" },
    ...
    { "usersetting_misc_enablemotors", "false" },
...
function createClientSettingsFile ( doReturn, replace ) 
    if ( fileExists ( "@xml/settings3.xml" ) ) then
        if ( replace ) then
            fileDelete ( "@xml/settings3.xml" )
        else
            if ( doreturn ) then
                return xmlLoadFile ( "@xml/settings3.xml" )
            else
                return nil
            end
        end
    end
    local file = xmlCreateFile ( "@xml/settings3.xml", "settings" )
...
function getSetting ( name )
    local file = xmlLoadFile ( "@xml/settings3.xml" ) or createClientSettingsFile ( true, false )
    local rV = false
...
addEventHandler ( "onClientUserSettingChange", root, function ( setting, value )
    ...
    elseif ( setting == "usersetting_misc_enablemotors" ) then
        exports.bengines:toggleEngines( value )
    end
end )