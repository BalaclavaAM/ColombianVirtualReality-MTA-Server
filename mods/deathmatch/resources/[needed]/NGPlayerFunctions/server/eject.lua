				...
				if ej	then
					if ej ~= p	then
						if getPedOccupiedVehicle ( ej ) == veh	then
							...
							print ( nab.." is not in this vehicle", p, 255, 255, 0, true, 8 )
						end
					else 
						print ( "You cannot eject yourself.", p, 255, 255, 0, true, 8 )
					end
				else 
					print ( nab.." is not in this vehicle", p, 255, 255, 0, true, 8 )
				end
			else 
				print ( "You are not the driver of this vehicle", p, 255, 255, 0, true, 8 )
			end
		else 
			print ( "You are not in a vehicle", p, 255, 255, 0, true, 8 )
		end
	else 
		print ( "/eject [player]", p, 255, 255, 0, true, 8 )
	end
end
addCommandHandler ( "eject", ejectPlayer ,false,false)
addCommandHandler ( "sacar", ejectPlayer ,false,false)