...
function jailPlayer ( p, dur, announce, element, reason )
	if( p and dur ) then
		...
		if ( announce ) then
			...
			local msg = ""
			if ( element and reason ) then
				msg = getPlayerName ( p ).." ha sido encarcelado por "..getPlayerName ( element ).." durante "..tostring ( dur ).." segundos ("..reason..")"
			elseif ( element ) then
				msg =  getPlayerName ( p ).." ha sido encarcelado por "..getPlayerName ( element ).." durante "..tostring ( dur ).." seconds"
			end
...
function unjailPlayer ( p, triggerClient )
	...
	exports['NGMessages']:sendClientMessage ( "Has sido liberado de la cárcel! Pórtate bien.", p, 0, 255, 0 )
	jailedPlayers[p] = nil
	setElementData ( p, "NGPolice:JailTime", nil )
	setElementData ( p, "isGodmodeEnabled", nil )
	exports['NGLogs']:outputActionLog ( getPlayerName ( p ).." ha sido liberado" )
...
function antiEscape()
	for i, v in pairs ( jailedPlayers ) do 
		if i and isElement(i) then
			if not(isPlayerJailed(i)) or getElementDimension(i)~=33 then
				local t = tonumber ( getElementData ( i, 'NGPolice:JailTime' ) ) or v
				jailPlayer ( i, tonumber ( t ), false )
			end
		end
	end
end
setTimer(antiEscape,1000,0)