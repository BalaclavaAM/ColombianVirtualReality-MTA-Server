...
db_exec ( [[CREATE TABLE IF NOT EXISTS accountdata ( Username VARCHAR(200), Email VARCHAR(200), Money INT, Armour INT, Health INT, x VARCHAR(20), 
...
function createAccount ( account, email )
	if ( account and type ( account ) == 'string' ) then
		...
		return db_exec ([[INSERT INTO `accountdata` (`Username`, `Email`,`Money`, `Armour`, `Health`, `x`, `y`, `z`, 
		...
		VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? );]], 
			account, email, '0', '0', '100', '0', '0', '0', '0', '0', '0', 'UnEmployed', 'UnEmployed', '0', '0', '0', 
...
function savePlayerData ( p, loadMsg, deleteTime )
	if ( p and getElementType ( p ) == 'player' ) then
		if ( not isGuestAccount ( getPlayerAccount ( p ) ) ) then
			...
			local prodskin = getElementData(p,"prodSkin") or false
			if prodskin then
				skin=tostring ( getElementData ( p, "NGUser.UnemployedSkin" ) ) or 28
			end
			...
			if not isPedDead(p) then
				...
				weapons = toJSON(globalWeapons[p])
	...
	if ( data and type ( data ) == 'table' ) then
		for i, v in ipairs  ( data ) do
			if ( v['Username'] == acc ) then
				...
				if ( jt > 0 ) then 
					exports['NGPolice']:jailPlayer ( p, jt, false ) 
				else
					local houseSpawn = getAccountData(getPlayerAccount(p),"houseSpawn")
					if houseSpawn then
						houseSpawn=fromJSON(houseSpawn)
						x=houseSpawn[1]
						y=houseSpawn[2]
						z=houseSpawn[3]
						interior=0
...
function email_exist ( email )
	if ( email ) then
		local q = db_query ( "SELECT * FROM accountdata WHERE Email='"..email.."' LIMIT 1" )
		if ( type ( q ) == 'table' ) then
			if ( #q > 0 ) then
				return q
			end
...
function set_email(acc,email)
	if (acc and email) then
		local q = db_query("UPDATE `accountdata` SET `Email` = '"..email.."' WHERE `accountdata`.`Username` ='"..acc.."';")
	end
end
...
function saveAllData ( useTime )
	...
	if ( useTime ) then
		...
		setTimer ( function ( )
			...
			if ( getResourceState ( getResourceFromName ( "NGGroups" ) ) == "running" ) then exports.NGGroups:saveGroups ( ) end 
			if getTickCount() - resetTime >= 24*60*60*1000 then 
				if tonumber(os.date("%H%M")) >= 930 then
					shutdown ("Reiniciando servidor...")
				end
			end
		end, 5000, 1 )
	else
...
addEventHandler( "onResourceStart", resourceRoot,
	function()
    	for k,v in ipairs(getElementsByType("player")) do
			addEventHandler ( 'onPlayerQuit', v, quitHandler )
		end
	end
)
...
addEventHandler("onPlayerLogin", root,
  function()
    addEventHandler ( 'onPlayerQuit', source, quitHandler )
  end
)

addEventHandler("onPlayerQuit", root,
  function()
    removeEventHandler ( 'onPlayerQuit', source, quitHandler )
  end
)


function removeQuit()
	local rm = removeEventHandler("onPlayerQuit", source, quitHandler)
	if rm then
		outputDebugString("Removido satisfactoriamente")
	end
end
addEvent("NGSQL:QuitarHandlerQuit",true)
addEventHandler("NGSQL:QuitarHandlerQuit",root,removeQuit)

function addQuit()
	addEventHandler("onPlayerQuit", source, quitHandler)
end
addEvent("NGSQL:PonerHandlerQuit",true)
addEventHandler("NGSQL:PonerHandlerQuit",root,addQuit)
...
addEventHandler( "onPlayerWasted", root,
	function()
		local weapons = { }
		for i=1,12 do 
			if getPedWeapon(source,i)~=4 then
				weapons[i] = { getPedWeapon ( source, i ), getPedTotalAmmo ( source, i ) } 
			end
		end
		globalWeapons[source] = weapons
	end
)
