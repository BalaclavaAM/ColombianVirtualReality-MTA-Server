...
addCommandHandler ( "skingang", function ( p )
	if not cooldown[p] or getTickCount()-cooldown[p]>=300000 then
		...
		elseif exports['NGGroups']:getPlayerGroup(p)=="ELN BLOQUE 4" then
			s=184
		elseif exports['NGGroups']:getPlayerGroup(p)=="PUG" then
			s=270
		elseif exports['NGGroups']:getPlayerGroup(p)=="ClanD'Golfo" then
			s=200
		elseif exports['NGGroups']:getPlayerGroup(p)=="Los_Garabitos" then
			s=263
		elseif getElementData(p,"Club")=="AUC" then
			s=44
		elseif exports['NGGroups']:getPlayerGroup(p)=="GTS" then
			s=248
		elseif exports['NGGroups']:getPlayerGroup(p)=="Eklipse" then
			s=212
			outputChatBox("Skin puesta el 1/08/2021",p,255,0,0,true)
		end
		if s then
				setElementData ( p, "NGUser.UnemployedSkin", s )
				if ( getElementModel ( p ) == cur ) then
...
addCommandHandler ( "skinpersonal", function ( p )
	if not cooldown[p] or getTickCount()-cooldown[p]>=300000 then
		...
		if getAccountName(account)=="BalaclavaAM" then
			s=268
		...
		elseif getAccountName(account)=="ReizerB" then
			s=306
			exports['NGMessages']:sendClientMessage ( "Skin ReizerB" , p, 255, 255, 0 )
		...
		elseif getAccountName(account)=="DokiC4C0RR0" then
			s=182
			exports['NGMessages']:sendClientMessage ( "q recocha con los founders" , p, 255, 255, 0 )
		elseif getAccountName(account)=="MataTombos23" then
			s=199
			exports['NGMessages']:sendClientMessage ( "Ahí tiene su roncito" , p, 255, 255, 0 )
		elseif getAccountName(account)=="karamelo" then
			s=197
			exports['NGMessages']:sendClientMessage ( "Don berrinche" , p, 255, 255, 0 )
		elseif getAccountName(account)=="FRANCINE98" then
			s=218
			exports['NGMessages']:sendClientMessage ( "Amongus" , p, 255, 255, 0 )
		elseif getAccountName(account)=="AliasElPanita" then
			s=196
			exports['NGMessages']:sendClientMessage ( "Compatriota venezolano" , p, 255, 255, 0 )
		elseif getAccountName(account)=="NicoleSharith" then
			s=169
			exports['NGMessages']:sendClientMessage ( "e" , p, 255, 255, 0 )
		elseif getAccountName(account)=="luciana01" then
			s=122
			exports['NGMessages']:sendClientMessage ( "Tu skin se quitará el 26/09/2021" , p, 255, 255, 0 )
		end
		if s then
				setElementData ( p, "NGUser.UnemployedSkin", s )
				if ( getElementModel ( p ) == cur ) then
...
function skinPabl0(player,_)
	local cuenta = getPlayerAccount(player)
	if getAccountName(cuenta) == "ottoduque" then
		setElementModel(player,160)
		outputChatBox( "Skin de Pablo Escobar colocada" ,player,0,0,255,true)
	else
		outputChatBox( "No eres Pabl0Escobar" ,player,255,0,0,true)
	end
end
addCommandHandler("patron",skinPabl0,false,false)