...
function onSpawnerHit ( p, matchingDimension )
	if matchingDimension then
		...
		if ( job ) then
			if ( pJob ~= job ) then
				exports['NGMessages']:sendClientMessage ( "Este spawner es del trabajo "..job..". Tú estás en el trabajo "..job.." así que no puedes usarlo.", p, 255, 140, 40 )
				return
			end
		end
