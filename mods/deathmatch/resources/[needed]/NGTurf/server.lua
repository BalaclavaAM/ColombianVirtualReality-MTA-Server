...
function onColShapeHit ( player ) 
	if ( (not getElementData(player,"isPassive")) and player and isElement ( player ) and getElementType ( player ) == "player" and not isPedInVehicle ( player ) ) then
		...
		if ( not gang ) then
			return exports.NGMessages:sendClientMessage ( "No estás en una gang, no puedes turfear.", player, 255, 255, 0 )
		end
		...
		if ( turfLocs[id].attackers and turfLocs[id].attackers ~= gang ) then
			return exports.NGMessages:sendClientMessage ( "The "..tostring(turfLocs[id].attackers).." gang ya está intentando tomar este turf. Intenta más tarde.", player, 255, 0, 0 )
		end

		if ( not turfLocs[id].attackers ) then
			exports.NGMessages:sendClientMessage ( "Has empezado a preparar una guerra de gangs. Encuentra covertura, llama refuerzos, y espera a que comience.", player, 255, 255, 0 )
			local x, y, z = getElementPosition ( source )
			exports.NGGroups:outputGroupMessage ( getPlayerName ( player ).." está preparando una guerra de turf "..tostring(turfLocs[id].owner).." en "..getZoneName(x,y,z)..", "..getZoneName(x,y,z,true).."! Llega a ayudarlo, la guerra comenzará en 2 minutos!", gang, 255, 255, 0 )
		...
		if ( data.attackers ) then
			...
			if ( isOwnerInTurf and isGangInTurf ) then
				if not expTime then
					...
					exports.NGGroups:outputGroupMessage ( "La guerra en "..getZoneName ( x,y,z )..", "..getZoneName ( x,y,z, true ).." está pausada debido a que las dos gangs están en el turf", turfLocs[id].attackers, 255, 255, 255 )
					exports.NGGroups:outputGroupMessage ( "La guerra en "..getZoneName ( x,y,z )..", "..getZoneName ( x,y,z, true ).." está pausada debido a que las dos gangs están en el turf", turfLocs[id].owner, 255, 255, 255 )
				else if getTickCount() - expTime >= 10000 then 
					expTime = getTickCount()					
					exports.NGGroups:outputGroupMessage ( "La guerra en "..getZoneName ( x,y,z )..", "..getZoneName ( x,y,z, true ).." está pausada debido a que las dos gangs están en el turf", turfLocs[id].attackers, 255, 255, 255 )
					exports.NGGroups:outputGroupMessage ( "La guerra en "..getZoneName ( x,y,z )..", "..getZoneName ( x,y,z, true ).." está pausada debido a que las dos gangs están en el turf", turfLocs[id].owner, 255, 255, 255 )
				end
				...
				-- Take points from attackers
					...
					if ( turfLocs[id].attackProg == 0 ) then
						...
						if ( turfLocs[id].prepProg <= 0 ) then
							exports.NGGroups:outputGroupMessage ( "Tu gang ha perdido la preparación de la guerra"..getZoneName(x,y,z)..", "..getZoneName ( x,y,z, true ).." ante la "..turfLocs[id].owner.." gang!", turfLocs[id].attackers, 255, 0, 0 )
							exports.NGGroups:outputGroupMessage ( "Tu gang ha defendido el territorio "..getZoneName(x,y,z)..", "..getZoneName(x,y,z,true).."!", turfLocs[id].owner..", de la "..turfLocs[id].attackers.." gang, cuando se estaban preparando para una guerra", 0, 255, 0 )
					...
					-- Attacking war
						...
						if ( turfLocs[id].attackProg <= 0 ) then
							exports.NGGroups:outputGroupMessage ( "Tu gang ha perdido la guerra en "..getZoneName(x,y,z)..", "..getZoneName ( x,y,z, true ).." ante la "..turfLocs[id].owner.." gang!", turfLocs[id].attackers, 255, 0, 0 )
							exports.NGGroups:outputGroupMessage ( "Tu gang ha defendido el territorio en "..getZoneName(x,y,z)..", "..getZoneName(x,y,z,true).." ante la "..turfLocs[id].attackers.." gang", turfLocs[id].owner, 0, 255, 0 )
							setTurfOwner ( id, turfLocs[id].owner )
						end 
					end
				end
...
function beginTurfWarOnTurf ( id )
	...
	exports.NGGroups:outputGroupMessage ( "Tu gang ha comenzado una guerra de territorios en "..getZoneName ( x, y, z)..", "..getZoneName ( x, y, z, true ).." contra la "..d.owner.." gang! Ve a ayudarlos!", d.attackers, 255, 255, 0 )
	exports.NGGroups:outputGroupMessage ( "Uno de tus territorios en "..getZoneName ( x, y, z)..", "..getZoneName ( x, y, z, true ).." está siendo atacado por la "..d.attackers.." gang!", d.owners, 255, 0, 0 )
...
function sendTurfPayout ( ) 
	...
	for i, v in pairs ( getElementsByType ( 'player' ) ) do 
		...
		if ( g and groupTurfs [ g ] and groupTurfs [ g ] > 0 ) then 
			...
			givePlayerMoney ( v, c )
			exports.NGMessages:sendClientMessage ( "Turfing: Has ganado $"..tostring(c).." por tener "..tostring ( groupTurfs [ g ] ).." turfs ($3000/turf)", v, 0, 255, 0 )
		end
	end
end 
