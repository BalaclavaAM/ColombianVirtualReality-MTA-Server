...
function addVehicleFromId ( id, price, description )
	...
	if ( query and query [ 1 ] and price and description ) then 
		...
		if ( query.Visible == 0 and query.Impounded == 0 ) then 
			...
			temp.color = { r=tonumber(color[1]), g=tonumber(color[2]), b=tonumber(color[3]), r1=tonumber(color[4]), g1=tonumber(color[5]), b1=tonumber(color[6]), r2=tonumber(color[7]), g2=tonumber(color[8]), b2=tonumber(color[9]), r3=tonumber(color[10]), g3=tonumber(color[11]), b3=tonumber(color[12])}
...
function setVehicleOwner ( id, account )
	...
	local color = toJSON ( table.concat({d.color.r,d.color.g,d.color.b, d.color.r1,d.color.g1,d.color.b1, d.color.r2,d.color.g2,d.color.b2, d.color.r3,d.color.g3,d.color.b3},", " ) );
	...
	aux_id = exports.ngsql:db_query ("SELECT MAX(VehicleID) as maximun FROM vehicles")[1]["maximun"]+1
	exports.ngsql:db_exec ( "INSERT INTO vehicles ( Owner, VehicleID, ID, Color, Upgrades, Position, Rotation, Health, Visible, Fuel, Impounded, Handling ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )",
		owner, aux_id, id, color, upgrades, position, rotation, health, visible, fuel, impounded, handling );
...
addEvent( "NGShops->UsedVehicles->onPlayerTryRecoverVehicle", true );
addEventHandler ( "NGShops->UsedVehicles->onPlayerTryRecoverVehicle", root, function ( id )
	if ( not usedVehicles [ id ] ) then 
		return exports.ngmessages:sendClientMessage ( "This vehicle is no longer available!", source, 200, 100, 130 );
	end 
	...
	if ( getAccountName(acc) ~= data.seller ) then
		return exports.ngmessages:sendClientMessage ( "No eres el dueño de este vehiculo", source, 200, 100, 130 );
	end
	...
	exports.ngmessages:sendClientMessage ( "Has recuperado tu vehiculo de la tienda "..getVehicleNameFromModel(data.vehicle_id).." for $"..data.price.."!", source, 0, 255, 0 );
