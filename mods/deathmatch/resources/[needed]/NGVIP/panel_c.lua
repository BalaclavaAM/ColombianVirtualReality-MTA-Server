...
function()
	...
	buttonMiniHedit = guiCreateButton(0.07, 0.63, 0.29, 0.07, "Mini Handling Editor", true, VipMain)
	guiSetProperty(buttonMiniHedit, "NormalTextColour", "FFFFFFFF")
	buttonModReset = guiCreateButton(0.42, 0.63, 0.20, 0.05, "Reset", true, VipMain)
...
function()
	if isTimer(healthTimer) then
		...
		dxDrawText((remain/1000).." sg", screenW * 0.25, screenH * 0.9200, (screenW * 0.9036) + 175, ( screenH * 0.7200) + 55, tocolor(255, 0, 0, 255), (3.00*xW), "default", "left", "top", false, false, false, false, false)

    end
	if isTimer(armourTimer) then
		local remain,times,interval = getTimerDetails(armourTimer)
        dxDrawText((remain/1000).." sg", screenW * 0.45, screenH * 0.9200, (screenW * 0.9036) + 175, ( screenH * 0.7942) + 55, tocolor(0, 255, 0, 255), 3.00*xW, "default", "left", "top", false, false, false, false, false)
	end
	if isTimer(fixTimer) then
		local remain,times,interval = getTimerDetails(fixTimer)
        dxDrawText((remain/1000).." sg", screenW * 0.65, screenH * 0.9200, (screenW * 0.9036) + 175, ( screenH * 0.6458) + 55, tocolor(0, 0, 160, 255), 3.00*xW, "default", "left", "top", false, false, false, false, false)
	end
end
)
