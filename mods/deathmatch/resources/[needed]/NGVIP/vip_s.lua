...
addCommandHandler ( "vipchat", function ( p, cmd, ... )
	if ( not isPlayerVIP ( p ) ) then
		return exports.NGMessages:sendClientMessage ( "Este comando es sólo para vips", p, 255, 0, 0 )
	end
	
	if ( isPlayerMuted ( p ) ) then
		return exports.NGMessages:sendClientMessage ( "Este comando está desactivado cuando estás muteado", p, 255, 255, 0 )
	end
	
	if ( not getElementData ( p, "userSettings" )['usersetting_display_vipchat'] ) then
		return exports.NGMessages:sendClientMessage ( "Por favor activa el vip chat en tu teléfono para usar este comando..", 0, 255, 255 )
	end
	
	local msg = table.concat ( { ... }, " " )
	if ( msg:gsub ( " ", "" ) == "" ) then
		return exports.NGMessages:sendClientMessage ( "Syntax: /"..tostring(cmd).." [mensaje]", p, 255, 255, 0 )
	end
...
function checkPlayerVipTime ( p )
	...
	local expDate = getElementData ( p, "NGVIP.expDate" )  or "2019-12-01"  -- Format: YYYY-MM-DD
	if ( isDatePassed ( expDate ) ) then
		setElementData ( p, "VIP", "None" )
		setElementData ( p, "NGVIP.expDate", "2019-12-01" )
...
function getVipLevelFromName ( l )
	local levels = { ['none'] = 0, ['bronze'] = 1, ['silver'] = 2, ['gold'] = 3, ['diamond'] = 4, ['esmeralda'] = 5 }
...
------------------------------------------
local payments = { [1] = 5000, [2] = 7000, [3] = 10000, [4] = 20000, [5] = 40000 }

VipPayoutTimer = setTimer ( function ( )
	exports.NGLogs:outputServerLog ( "Enviando dinero VIP...." )
	print_ ( "Enviando dinero VIP...." )
	outputDebugString ( "Enviando dinero VIP" )
	...
	for i, v in ipairs ( getElementsByType ( "player" ) ) do
		if ( isPlayerVIP ( v ) ) then
			...
			exports.NGMessages:sendClientMessage ( "Acá hay $"..money.." por ser un jugador VIP!", v, 0, 255, 0 )
		end
	end
end, (60*60)*1000, 0 ) 
...
function isDatePassed ( date )
	...
	elseif ( this.year == old.year and this.month > old.month ) then
		return true
	elseif ( this.year == old.year and this.month == old.month and this.day > old.day ) then --da un error 
		return true
	end
	return false
end
