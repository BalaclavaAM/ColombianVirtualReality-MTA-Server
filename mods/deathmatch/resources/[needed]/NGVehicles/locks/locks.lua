...
addCommandHandler ( "lock", function ( p )
	...
	if not cooldown[p] or getTickCount() - cooldown[p]>5000 then
		for i, v in pairs ( vehicles ) do 
			if ( isElement(v) and getElementData ( v, "NGVehicles:VehicleAccountOwner" ) == acc ) then
		...
		exports['NGMessages']:sendClientMessage ( "No spamees!.", p, 255, 255, 0 )
		return false, nil
	end
