function greetings()
	outputChatBox("#F20A40 De parte de CVR te deseamos feliz 2021!",255,255,255,true)
end
addEventHandler("onClientResourceStart",resourceRoot,greetings)

function firework(command,bt)
	local bt = tonumber(bt)
	if (getPlayerMoney(localPlayer) < 10000 ) then return outputChatBox("No tienes dinero suficiente (10k).") end 
	triggerServerEvent("takeCueteServerMoney", resourceRoot)
	if bt and bt >= 1 and bt <= 6 then
		outputChatBox("Selecciona el tipo de fuegos: "..bt,50,200,100,true)
		if bt == 1 then
			createBox(bt,33)
		elseif bt == 2 then
			createBox(bt,25,935)
		...
		outputChatBox("ERROR: El síntaxis es /cuete 1 - 4 ej: /cuete 1",200,0,0,true)
	end
end
addCommandHandler("cuete",firework,false,false)
