...
function client_CheckPermissions()
    ...
    if isObjectInACLGroup ("user."..accName, aclGetGroup ( "Admin" ) ) then --hasObjectPermissionTo ( client, "function.banPlayer" ) == true then
		isAdminStatus[client] = true
	else--if hasObjectPermissionTo ( client, "function.banPlayer" ) == false then
		isAdminStatus[client] = false
	end
	triggerClientEvent ( client, "client_ReturnPermissions", client, isAdminStatus[client] )
end

function client_ReturnPrice()
	triggerClientEvent ( client, "client_ReturnPrecio", client, get("precio") )
end
...
function client_CallSpecificTeleport ( teleportID )
	...
	warpDesc = xmlNodeGetAttribute ( selectedNode, "desc" )
	triggerClientEvent ( client, "client_ReturnSpecificTeleport", client, warpX, warpY, warpZ, warpRot, warpType, warpDesc )
end

function decirelcaldo ( player, desc )
	exports['NGMessages']:sendClientMessage ("#FFFFFF[#fff200C#0015ffV#b00900R#FFFFFF]#54f7e2"..player.." #FFFFFFse teletransportó a #54f7e2" ..desc.."#009cad!",root,255,0,0)
end
addEvent("Caldo", true)
addEventHandler("Caldo", getRootElement(), decirelcaldo )

function quitarluquita ( player )
	takePlayerMoney(player, tonumber(get("precio")))
end
addEvent("quitarluquita", true)
addEventHandler("quitarluquita", getRootElement(), quitarluquita )

addEventHandler ( "client_CheckPermissions", getRootElement(), client_CheckPermissions )
addEventHandler ( "getPrecios",getRootElement(), client_ReturnPrice)
