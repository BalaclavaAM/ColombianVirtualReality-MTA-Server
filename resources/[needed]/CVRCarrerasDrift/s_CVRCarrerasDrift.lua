exports.NGSQL:db_exec([[CREATE TABLE IF NOT EXISTS carrerascvr ( NombreCarrera VARCHAR(200), Creador VARCHAR(50), Vehiculo INT, Checkpoints TEXT, Startcoords TEXT, MejoresTiempos TEXT )]]);
...
function getCarrera(nombre)
    local data = exports['NGSQL']:db_query ( "SELECT * FROM carrerascvr WHERE NombreCarrera=?", nombre )
    if #data>0 then
        return data
    else
        return false
...
function getCarreras()
    local data = exports['NGSQL']:db_query ( "SELECT * FROM carrerascvr WHERE 1" )
    return data
end


function getCarrerasBy(nombre)
    local data = exports['NGSQL']:db_query ( "SELECT * FROM `carrerascvr` WHERE `NombreCarrera` LIKE '%"..nombre.."%'")
    return data
end

function delCarrera(nombre)
    if getCarrera(nombre)~=false then
        exports['NGSQL']:db_query ( "DELETE FROM `carrerascvr` WHERE `NombreCarrera`=?",nombre )
    end
end
...
function crearCarrera(nombre,checkpoints,startcoords,vehiculo)
    if nombre and checkpoints and startcoords and vehiculo then
        if not(getCarrera(nombre)) then
            ...
            exports['NGSQL']:db_exec ( "INSERT INTO carrerascvr ( `NombreCarrera`, `Creador`, `Vehiculo`, `Checkpoints`, `Startcoords`, `MejoresTiempos`) VALUES ( ?,?,?,?,?,? );", nombre, getAccountName(getPlayerAccount(source))..": "..getPlayerName(source),tonumber(vehiculo),toJSON(checkpoints),toJSON(startcoords),toJSON(tabla))
...
function cargarCarrera(p,nombrecarrera)
    if (getElementData(p,"VIP")=="Esmeralda" or getElementData(p,"VIP")=="Diamond" or (exports['NGAdministration']:isPlayerStaff ( p ))) and not(playersthatloaded[p]) then
        ...
        outputChatBox(prefix.."Tienes que ser VIP Diamond o más para cargar carreras.",p,255,0,0,true)
    end
end
...
function onPlayerWin()
    local handling1=getVehicleHandling(getPedOccupiedVehicle(source))
    local handling2=exports['NGVehicles']:getVehicleHandlingg(getElementModel(cars[source]))
    if getPedOccupiedVehicle(source) and getPedOccupiedVehicle(source)==cars[source] and (handling1['engineAcceleration']==handling2['engineAcceleration'] and handling1['tractionMultiplier']==handling2['tractionMultiplier'] and handling1['tractionLoss']==handling2['tractionLoss']) then
...
function forzarinicio(p)
    if participantes[p] and (getElementData(p,"VIP")=="Esmeralda" or getElementData(p,"VIP")=="Diamond" or (exports['NGAdministration']:isPlayerStaff ( p ))) and races[participantes[p]]['loadedby']==p then
...
function checkTime(p,race,time)
    if time<races[race]['mejores'][1]['tiempo'] then
        ...
        exports['NGSQL']:db_exec ( "UPDATE `carrerascvr` SET `MejoresTiempos`=? WHERE `NombreCarrera`=?",toJSON(races[race]['mejores']),race )
...
function entrarcarrera(p,_,carrera)
    if not(exports['NGPolice']:isPlayerJailed(p)) and not(isPedDead(p)) and not(getElementData(p,"tazed")) then
        if carrera then
            if tonumber(carrera) then
                ...
                if races[carrera] and not(races[carrera]['started']) and #races[carrera]['players']<#races[carrera]['inicio'] then
                    ...
                    outputChatBox(prefix.."¿Quieres otra? #3AFF00Pídele a un VIP Diamond que la active",p,255,0,0,true)
                end
        else
            local tabla={}
            for k,v in pairs(races) do
