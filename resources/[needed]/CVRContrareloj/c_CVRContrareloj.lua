...
local penal=0
local car=""
local thecheckpointTable={}
local currentCheckPointTable={}
local currentRace=""
...
function startRace(race)
    ...
    currentCheckPointTable={}
    currentCheckPointTable[1]=0
end
...
function endRace(race)
    ...
    triggerServerEvent("serverWin",localPlayer,localPlayer,time,besttime,race,currentCheckPointTable)
    if #thecheckpointTable<#currentCheckPointTable then
        outputChatBox("No tenías record de checkpoints registrados. Se registró tu última entrada!",0,255,0)
        triggerServerEvent("CVRContrareloj:UploadTable",localPlayer,currentCheckPointTable,race)
    elseif thecheckpointTable[#thecheckpointTable] and currentCheckPointTable[#thecheckpointTable] and currentCheckPointTable[#thecheckpointTable] < thecheckpointTable[#thecheckpointTable] then
        outputChatBox("Récord de tabla mejorado. Guardando!",0,255,0)
        triggerServerEvent("CVRContrareloj:UploadTable",localPlayer,currentCheckPointTable,race)
    end
end
...
function createDisplay()
    ...
    dxDrawRectangle(screenW * 0.7885, screenH * 0.5792, screenW * 0.1708, screenH * 0.1700, tocolor(33, 15, 94, 125), false)
    dxDrawText("Tiempo Actual", screenW * 0.7937, screenH * 0.5875, screenW * 0.8984, screenH * 0.6083, tocolor(255, 255, 255, 254), 1.00, "bankgothic", "left", "top", false, false, false, false, false)
    dxDrawText(tiempodesc, screenW * 0.7937, screenH * 0.6142, screenW * 0.8969, screenH * 0.6275, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, false, false, false)
    dxDrawText("Mejor Tiempo", screenW * 0.7937, screenH * 0.6275, screenW * 0.8984, screenH * 0.6483, tocolor(255, 255, 255, 255), 1.00, "bankgothic", "left", "top", false, false, false, false, false)
    dxDrawText(besttimedesc, screenW * 0.7953, screenH * 0.6533, screenW * 0.8984, screenH * 0.6667, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, false, false, false)
    dxDrawText("Tiempo Checkpoint", screenW * 0.7937, screenH * 0.6700, screenW * 0.8984, screenH * 0.6908, tocolor(255, 255, 255, 255), 1.00, "bankgothic", "left", "top", false, false, false, false, false)
    dxDrawText(secondsToTimeDesc(currentCheckPointTable[index]), screenW * 0.7937, screenH * 0.6958, screenW * 0.9125, screenH * 0.7075, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, false, false, false)
    dxDrawText("Checkpoint "..index.."/"..#coords[currentRace], screenW * 0.7937, screenH * 0.7125, screenW * 0.8984, screenH * 0.7333, tocolor(255, 255, 255, 255), 1.00, "bankgothic", "left", "top", false, false, false, false, false)
end

function showPositiveNotification()
    local difference = (currentCheckPointTable[index]-thecheckpointTable[index])
    dxDrawText(difference.."sg", screenW * 0.7177, screenH * 0.6875, screenW * 0.7854, screenH * 0.7125, tocolor(23, 255, 0, 255), 1.00, "pricedown", "left", "top", false, false, false, false, false)
end

function showNegativeNotification()
    local difference = (currentCheckPointTable[index]-thecheckpointTable[index])
    dxDrawText("+"..difference.."sg", screenW * 0.7177, screenH * 0.6875, screenW * 0.7490, screenH * 0.7125, tocolor(255, 0, 0, 255), 1.00, "pricedown", "left", "top", false, false, false, false, false)
end
...
function markerHitHandler(hitPlayer,matchingDimension)
    if hitPlayer == localPlayer then
        ...
        currentCheckPointTable[index+1]=(time/1000)
        if thecheckpointTable[index+1] and currentCheckPointTable[index+1] and currentCheckPointTable[index+1]<thecheckpointTable[index+1] then
            addEventHandler("onClientRender",root,showPositiveNotification)
            setTimer(function() removeEventHandler("onClientRender",root,showPositiveNotification) end,1000,1)
        elseif thecheckpointTable[index+1] and currentCheckPointTable[index+1] then
            addEventHandler("onClientRender",root,showNegativeNotification)
            setTimer(function() removeEventHandler("onClientRender",root,showNegativeNotification) end,1000,1)
        end
...
function getGreatestTime(greatestTime,checkpointTable)
    besttime=tonumber(greatestTime)
    checkpointTable=fromJSON(checkpointTable)
    besttimedesc=secondsToTimeDesc(tonumber(greatestTime)/1000)
    if checkpointTable then
        thecheckpointTable=checkpointTable
    else
        thecheckpointTable={}
    end
end
...
addEvent("CreateClientCheckpointsContrareloj",true)
addEventHandler("CreateClientCheckpointsContrareloj",root,startRace)    