...
function makeItToServer2(thePlayer,job,vehiculo,dimension,frzn,password,players,desc,mp)
	crearEvento(thePlayer,job,vehiculo,dimension,frzn,password,players,desc,mp)
end
...
function ( thePlayer ) 
	if ((not carrazosevento[thePlayer] and entradosevento[thePlayer]) or canCreateEvent(thePlayer)) and estadoevento == 1 then
		if not ecar then 
			return 
		end
