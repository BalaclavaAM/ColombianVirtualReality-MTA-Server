...
function drawMainTele()
	...
	local wWidth = 0.45
	local wHeight = 0.45
	teleManager = guiCreateWindow ( wX, wY, wWidth, wHeight, "Administrador de Teleports", true ) guiWindowSetSizable ( teleManager, false ) guiSetVisible ( teleManager, false )
	
	teleManagerClose = guiCreateButton ( 0.25, 0.85, 0.5, 0.1, "Cerrar", true, teleManager )
	teleManagerAddTele = guiCreateButton ( 0.7, 0.15, 0.25, 0.1, "Añadir Teleport", true, teleManager )
	teleManagerDeleteTele = guiCreateButton ( 0.7, 0.3, 0.25, 0.1, "Eliminar Teleport", true, teleManager )
	teleManagerUseTele = guiCreateButton ( 0.7, 0.6, 0.25, 0.15, "Usar Teleport", true, teleManager )
	
	listLabel = guiCreateLabel ( 0.05, 0.1, 0.5, 0.1, "Teleports Disponibles:", true, teleManager )
	
	list = guiCreateGridList ( 0.05, 0.15, 0.6, 0.6, true, teleManager ) guiGridListSetSortingEnabled ( list, false )
	listDescColumn = guiGridListAddColumn ( list, "Descripción", 0.6 )
	listTypeColumn = guiGridListAddColumn ( list, "Tipo de Teleport", 0.3 )
end
